 ...
 * Must be called outside any transaction and from inside the PosApi serial queue — recordPinFailure's read-modify-
 * write of the guard row is only safe under that single-writer guarantee (review M-3).
 */
export async function login(db: RemoteDb, deps: ApiDeps, userId: string, pin: string): Promise<UserDto> {
  ...
  await assertNotLocked(db, userId, guard, deps.now()) // while locked the PIN is not even checked
...
/**
 * spec §4.3: voiding a paid order needs an owner's PIN — same lockout counter as login (D50 Q3-21). Same caller
 * requirement as `login` (review M-3): outside any transaction, from inside the PosApi serial queue.
 */
