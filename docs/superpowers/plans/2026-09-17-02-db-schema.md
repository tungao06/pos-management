...
- **CHECK constraint อนุญาต (D47 ข้อ 7):** ใช้ `check(name, sql\`...\`)` ของ drizzle ในทั้งสอง dialect เมื่อ generate ได้จริง (ยืนยันแล้วว่า drizzle-kit 0.31 สร้าง SQL CHECK ให้ทั้ง sqlite และ pg) ตั้งชื่อ constraint ลงท้าย `_ck` และให้ parity/drift รู้จัก (parity ไม่เทียบ CHECK โดยตรงแต่ drift เทียบกับ migration ที่ generate ใหม่เสมอ) · ถ้า dialect ใดสร้าง CHECK ให้ไม่ได้ ให้ใช้ migration ไฟล์ `--custom` แทนและปรับเทสต์ parity/drift ให้รับรู้
...
จำนวนเทสต์ของ `@dayo/db-schema` หลังแต่ละ task (ใช้ตรวจว่าไม่มีเทสต์หาย): Task 1 = 20 · Task 2 = 28 · Task 3 = 38 · Task 4 = 40 · Task 5 = 42 · Task 6 = 48 · หลังแก้ final review = 94 (parity 43 · migrate 8 · seed 15 · constraints 11 · triggers 15 · drift 2) · **หลังแก้ D47 ข้อ 5–9 = 112** (parity 43 · migrate 8 · seed 15 · constraints 29 · triggers 15 · drift 2)
...
git commit -m "feat(contracts,domain): VOID_REFUND cash movement and voidRefundsSatang (D36)"
```
...
git commit -m "feat(domain): event hash covers orderId, per-order seq and deviceId (D38)"
```
...
git commit -m "feat(contracts,excel-import): equipment life in whole months, regenerate seed (D37)"
```
...
git commit -m "feat(db-schema): reference tables for sqlite and pg with parity test"
```
...
git commit -m "feat(db-schema): stock transaction tables"
```
...
```bash
git add packages/db-schema
git commit -m "feat(db-schema): order, payment, event chain, shift and cash tables"
```
...
```bash
git add packages/db-schema
git commit -m "feat(db-schema): audit log, outbox/sync_state and server-only tables"
```
...
```bash
git add packages/db-schema
git commit -m "feat(db-schema): generated migrations for sqlite and pg with migration tests"
```
...
```bash
git add packages/db-schema
git commit -m "feat(db-schema): seed loader for sqlite and pg from the Excel-derived seed"
```
...
git commit -m "docs: plan 2 execution notes and mark plan 2 complete"
```
...
## แก้ตาม D47 ข้อ 5–9 (18 ก.ย. 2026)

เจ้าของตอบ "ทำต่อเลย" (D47) ให้ค่าแนะนำของข้อ 5–9 ที่ค้างจาก final review (M4 CHECK, M9 version/updated_at, M10 setting PK) ครบ — ทำทั้งสอง dialect พร้อมเทสต์ที่พิสูจน์ว่า DB ปฏิเสธแถวที่ผิดจริง จำนวนเทสต์ `@dayo/db-schema` จาก 94 → **112** (เพิ่มใน `test/constraints.test.ts` ทั้งหมด 11 → 29)

- **ข้อ 5 (`order`):** เพิ่ม `unique(device_id, business_date, queue_no)` ทั้งสอง dialect (คู่กับ `unique(device_id, receipt_no)` เดิม) — เลขคิวซ้ำได้ถ้าคนละเครื่องหรือคนละวัน
- **ข้อ 6 (`shift`):** partial unique index `shift_open_uq` บน `device_id` `WHERE status = 'open'` — เปิดกะพร้อมกันได้หลายเครื่อง แต่เครื่องเดียวเปิดซ้อนไม่ได้ (เทียบแบบเดียวกับ `bom_current_uq`/`recipe_current_uq`)
- **ข้อ 7 (CHECK):** เพิ่มบรรทัดใน Global Constraints อนุญาต `check()` ของ drizzle (ยืนยันแล้วว่า drizzle-kit 0.31 generate SQL CHECK ได้ทั้งสอง dialect จึงไม่ต้องใช้ `--custom`) · เพิ่ม CHECK:
  - `cash_movement`: `(kind = 'VOID_REFUND') = (order_id is not null)` และ `amount_satang > 0`
  - `order`: `subtotal_satang >= 0` · `discount_satang >= 0` · `total_satang >= 0` · `discount_satang <= subtotal_satang`
  - `payment.amount_satang > 0` · `stock_movement.qty_milli <> 0` · `order_line.qty > 0`
- **ข้อ 8 (version/updated_at):** ทุกตาราง R (18 ตารางใน `reference.ts`) มี `version int NOT NULL DEFAULT 1` และ `updated_at text NOT NULL` · `user` มีอยู่แล้วจาก Task 0 (เติม `.default(1)` ให้ตรงกัน) · `bom` และ `recipe` มีคอลัมน์ `version` อยู่แล้วแต่เป็นเลขรุ่นของสูตร/BOM เอง (แก้ = insert แถวใหม่ ไม่ใช่ UPDATE ทับ) — ตัดสินใจว่าคอลัมน์เดิมนี้ทำหน้าที่ตรงตามที่ข้อ 8 ต้องการอยู่แล้ว (int NOT NULL เริ่มที่ 1) จึงไม่เพิ่มคอลัมน์ชื่อซ้ำ เติมแค่ `.default(1)` และ `updated_at` ให้สองตารางนี้ (ไม่ใช่การตัดสินใจเชิงธุรกิจ เป็นรายละเอียด implementation เพื่อเลี่ยงชื่อคอลัมน์ชนกัน) · `seedToRows` ใส่ `version: 1, updatedAt: opts.now` ให้ทุกแถวที่มาจาก seed
- **ข้อ 9 (`setting`):** PK เปลี่ยนจาก `key` เดี่ยวเป็น `(key, effective_from)` ด้วย `primaryKey({ columns: [t.key, t.effectiveFrom] })` — เก็บประวัติค่าตั้งค่าได้เหมือน `price`/`recipe` · ไม่มีโค้ดอื่นในโปรเจกต์ที่อ้างอิง PK เดิม (ยังไม่มี `apps/*`) จึงไม่กระทบที่อื่น
- **migration:** ยังไม่มีอะไร deploy จึง regenerate baseline เดียวต่อ dialect แบบเดียวกับรอบ final review (ลบ `drizzle/{sqlite,pg}` เดิม รัน `drizzle-kit generate` ใหม่ เปลี่ยนชื่อไฟล์/`_journal.json` tag เป็น `0000_baseline`) แล้วสร้าง `--custom` migration ว่างซ้ำ (`0001_append_only` sqlite · `0001_server_seq_bump`, `0002_append_only` pg) แล้วคัดลอกเนื้อ trigger เดิมกลับเข้าไปทุกตัวอักษร (รายชื่อตารางที่มี `server_seq`/append-only ไม่เปลี่ยนจาก D47 ข้อ 5–9 เพราะไม่มีตาราง R/T เพิ่มหรือหาย) · `test/drift.test.ts` ผ่านเขียว
- **เทสต์ใหม่ (`test/constraints.test.ts`):** สอง describe block ใหม่ต่อ dialect ("D47 items 5–9") พิสูจน์ว่า DB ปฏิเสธจริง — กะที่สองเปิดพร้อมกันบนเครื่องเดิมถูกปฏิเสธแต่คนละเครื่องผ่าน · เลขคิวซ้ำบนเครื่อง/วันเดิมถูกปฏิเสธแต่คนละเครื่องผ่าน · `VOID_REFUND` ไม่มี `order_id` ถูกปฏิเสธ · `PAID_IN` มี `order_id` ถูกปฏิเสธ · ยอดติดลบ/discount เกิน subtotal ของ `order` ถูกปฏิเสธ · `payment`/`order_line`/`stock_movement` ปฏิเสธค่าที่ผิดเงื่อนไข CHECK · `setting` คีย์เดิมคนละ `effective_from` ผ่าน แต่คู่ซ้ำถูกปฏิเสธ · เทสต์เพิ่มว่าทุกตาราง R มี `version`/`updated_at` จริงในทั้งสอง dialect
