...
- Produces:
  ...
  - `PosApi.getOrder(orderId: string): Promise<OrderDetailDto>` — throw `ORDER_NOT_FOUND` · `voidable` = `status === 'paid'` และบิลอยู่ในกะที่เปิดอยู่ (รอคำตอบ Q3-13)
...
describe('listOrders / getOrder', () => {
  ...
  it('orders of a closed shift are not listed and not voidable (รอคำตอบ Q3-13)', async () => {
    const t = await openReadyApi()
...
describe('promptPayForAmount', () => {
  it('builds the dynamic QR for the configured PromptPay id (รอคำตอบ Q3-4)', async () => {
    const t = await openReadyApi()
...
export async function getOrder(db: RemoteDb, orderId: string): Promise<OrderDetailDto> {
  ...
  return {
    ...
    // Only paid orders of the open shift can be voided (รอคำตอบ Q3-13)
...
```ts
/** EMVCo payload for the shop's PromptPay id with the bill amount (spec §5, D7 · รอคำตอบ Q3-4). Built offline. */
export async function promptPayForAmount(db: RemoteDb, amountSatang: number): Promise<string> {
  ...
  const id = await getSetting(db, PROMPTPAY_SETTING_KEY)
...
```ts
    ...
    promptPayForAmount: (amountSatang) => serial(() => promptPayForAmount(db, amountSatang)),
```
...
```bash
git add apps/pos
git commit -m "feat(pos): order history queries and offline PromptPay payload for the bill amount" -m "Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>"
```
...
/** spec §5: full-screen PromptPay QR with the bill amount, confirmed by eye (รอคำตอบ Q3-4). */
...
/** spec §5: big queue number + items for the drink maker, closes itself after 5 s. On screen only, no paper (รอคำตอบ Q3-5). */
...
```bash
git add apps/pos
git commit -m "feat(pos): cash payment with change, PromptPay QR screen, queue-number screen and cash sale e2e" -m "Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>"
```
...
**Interfaces:**
- Consumes: `requireOwnerPin` (Task 6) · `voidReturnMovements`, `MovementDraft` (domain) · `insertMovements` (Task 10) · `loadCatalogRemote` (Task 4) · `appendOrderEvents`, `enqueueOutbox` (Task 5) · `getOrder` (Task 11) · `CashMovementKind` มี `VOID_REFUND` (D36, แผน 2)
- Produces:
  ...
  - ผลในฐานข้อมูล (spec §4.3, D18, D36, D39): `order.status = 'voided'`, `voided_at` · outbox `order:<id>:voided` · ยังไม่ทำ → movement `VOID_RETURN` (+ปริมาณเดิม ต้นทุนเดิม) + event `STOCK_RETURNED` · ทำแล้ว → ไม่มี movement, `waste: true` ใน `VOIDED` · เงินสด → `cash_movement` kind `VOID_REFUND` = Σ payment CASH ในกะที่เปิดอยู่ · QR → ต้องมี `refundReference` (รอคำตอบ Q3-15) บันทึกใน payload ของ `VOIDED`
...
describe('voidOrder', () => {
  it('cash, not made yet: returns every ingredient at the original cost, refunds cash as VOID_REFUND, chains VOIDED + STOCK_RETURNED', async () => {
    ...
    expect(cash[0]).toMatchObject({ shiftId: t.shift.id, kind: 'VOID_REFUND', amountSatang: 9000, createdBy: t.owner.id })
  ...
  it('PromptPay needs a refund transfer reference and records no cash movement (รอคำตอบ Q3-15)', async () => {
    const t = await openReadyApi()
  ...
  it('cannot void an order of a closed shift (รอคำตอบ Q3-13)', async () => {
    const t = await openReadyApi()
    const sale = await sellSku(t, 'Original-16oz', 1, { method: 'CASH', tenderedSatang: 4500 })
    t.raw.prepare("update shift set status = 'closed', closed_at = ?, closed_by = ? where id = ?").run(t.clock.now(), t.owner.id, t.shift.id)
    t.clock.set('2026-09-18T03:00:00.000Z')
...
export async function voidOrder(db: RemoteDb, deps: ApiDeps, input: VoidOrderInput): Promise<OrderDetailDto> {
  ...
  await db.transaction(async (tx) => {
    ...
    const shift = await currentOpenShift(tx, device.id)
    // Only paid orders of this device's open shift (รอคำตอบ Q3-13)
    ...
    if (qrRefundSatang > 0 && refundReference === '') throw new PosError('BAD_INPUT', 'a PromptPay void needs the refund transfer reference') // (รอคำตอบ Q3-15)
    ...
    if (!input.made) {
      ...
      returnedMovementIds = await insertMovements(
        ...
        await loadCatalogRemote(tx),
      )
    }

    let cashMovementId: string | null = null
    if (cashRefundSatang > 0) {
      const row = { id: deps.newId(), shiftId: shift.id, kind: 'VOID_REFUND', amountSatang: cashRefundSatang, reason: `${order.receiptNo ?? order.id}: ${reason}`, createdBy: actor.id, createdAt: at }
...
export type VoidOrderInput = {
  ...
  made: boolean
  /** Required when the order was paid by PromptPay (รอคำตอบ Q3-15). */
...
Run: `pnpm --filter @dayo/pos test && pnpm --filter @dayo/pos typecheck`
Expected: PASS ทั้งหมด · ถ้า insert `cash_movement` ตกเพราะ kind ถูกจำกัดด้วย CHECK/enum ที่ไม่มี `VOID_REFUND` แปลว่าแผน 2 ยังไม่ได้แก้ตาม D36 → หยุดและแจ้ง (Task 1 Step 1)

- [ ] **Step 4: Commit**

```bash
git add apps/pos
git commit -m "feat(pos): void paid orders with owner PIN, return-or-waste choice and VOID_REFUND cash movement" -m "Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>"
```
...
test('PromptPay sale shows a QR with the amount; void it with a refund reference and the other owner PIN', async ({ page }) => {
  ...
  expect(payload.startsWith('000201010212')).toBe(true) // dynamic QR (รอคำตอบ Q3-4)
...
export function VoidDialog({ order, onClose }: { order: OrderDetailDto; onClose: () => void }): JSX.Element {
  ...
  const submit = (pin: string): void => {
    ...
    if (isQr && refundRef.trim() === '') return setError(TH.errRefundRefRequired) // (รอคำตอบ Q3-15)
...
```bash
git add apps/pos
git commit -m "feat(pos): today's orders, order detail with event timeline, void dialog and QR/void e2e" -m "Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>"
```
...
- Produces: CI job `e2e` · บันทึกการทำแผน 3 · README แผนอัปเดต
...
git status --short
```
Expected: ทุกแพ็กเกจผ่าน · e2e 5 เทสต์ผ่าน (first-run, sell-cash, sell-qr-void ×2, offline) · `git status` ไม่มีไฟล์ `test-results/`, `playwright-report/`, `dist/` ค้าง (ถ้ามี ให้เพิ่ม 2 บรรทัด `test-results/` และ `playwright-report/` ใน `.gitignore` แล้ว commit แยก)

- [ ] **Step 4: ทดสอบบนแท็บเล็ตจริง (รอคำตอบ Q3-1, Q3-2)**
...
1. ตั้งค่าเครื่อง (ใช้หมายเลขพร้อมเพย์จริงของร้าน (รอคำตอบ Q3-3)) → ล็อกอิน → เปิดกะ
2. ขายเงินสด 3 บิล (มีส่วนลด 1 บิล) · ตรวจเงินทอนและเลขคิว 1–3
3. ขาย QR 1 บิล → **สแกนด้วยแอปธนาคารจริง** ตรวจว่าแอปขึ้นชื่อบัญชีร้านและยอดตรงบิล แล้วกดยกเลิกในแอปธนาคาร (ไม่ต้องโอนจริง)
4. ยกเลิกบิลเงินสด 1 บิลแบบ "ยังไม่ทำ" และ 1 บิลแบบ "ทำแล้ว" ด้วย PIN ของอีกคน
5. เปิดโหมดเครื่องบิน → ปิด/เปิดแอปจากไอคอนหน้าจอหลัก → ล็อกอิน → ขายเงินสด 1 บิล
6. รีสตาร์ตแท็บเล็ต → เปิดแอป → บิลวันนี้ครบทุกบิล สถานะถูกต้อง
7. ปล่อยไว้ 10 นาที → แอปล็อกเอง (spec §5)
8. จดเวลาโดยประมาณ: เปิดแอปจนเห็นหน้า PIN · กดยืนยันรับเงินจนเห็นเลขคิว
...
สถานะ: ✅/⚠️ · จำนวนเทสต์ (unit/integration/e2e) · branch · ยังไม่ push

## 1. ผล spike และการทดสอบบนแท็บเล็ต
ลิงก์ [ผล spike](2026-09-17-03-ผลspike.md) · ผลรายการ Task 15 Step 4 ข้อ 1–8 · เวลาที่วัดได้

## 2. คำตอบจากเจ้าของที่ใช้ (Q3-1 … Q3-19) และจุดที่แก้แผนตามคำตอบ

## 3. การตัดสินใจระหว่างทำที่ไม่ได้อยู่ในแผน

## 4. ส่งต่อแผน 3b (ปิดกะ/Z/สำรองไฟล์) และแผน 4–8
- ปิดกะต้องใช้ expected cash = opening + cash_sales − void_refunds (`cash_movement` kind `VOID_REFUND`) + paid_in − paid_out − drops (D36)
- แผน 5: seed id ใช้ `deterministicIds('dayo-seed-v1')` + `SEED_EFFECTIVE_FROM` เดียวกัน · ผู้ใช้/เครื่อง/ตั้งค่าจากหน้าตั้งค่าเครื่องยังไม่อยู่ใน outbox (T15) ต้องออกแบบการลงทะเบียน · outbox key รูปแบบ `<table>:<id>[:suffix]`
- สิ่งที่ยังไม่ได้ทำ/พักไว้
```

แก้แถวแผน 3 ใน `docs/superpowers/plans/README.md` เป็น:
```markdown
| 3 | [2026-09-17-03-pos-sell.md](2026-09-17-03-pos-sell.md) | 0, 3 | spike SQLite WASM/OPFS · แอปขายในเครื่อง: ตั้งค่าเครื่อง · ล็อกอิน · เปิดกะ · ขาย · เงินสด/QR · ตัดสต็อก · ประวัติ · ยกเลิก ([คำถาม](2026-09-17-03-คำถามก่อนทำ.md) · [บันทึก](2026-09-17-03-บันทึกการทำแผน3.md)) | ✅ เสร็จ YYYY-MM-DD |
| 3b | 03b-pos-shift-close | 3 | ปิดกะ · นับเงิน · Z report · สำรองไฟล์ฐานข้อมูล — **ต้องเสร็จก่อนใช้ขายจริง** (รอคำตอบ Q3-17) | รอ |
```
...
git commit -m "test(pos): offline selling e2e, CI e2e job and plan 3 close-out notes" -m "Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>"
```

ห้าม push — รอ audit + code review (opus final review ตาม D43) และเจ้าของสั่ง (D42)
...
| §8 spike 3 ข้อ (ทน reload/ปิดแอป · Drizzle ใน Worker + transaction · persistent storage) | Task 1 S3–S5, S9, S10 + ทางสำรอง F1–F4 |
| §10 migration ฝังในแอป รันตอนเปิดแอป | Task 1 (`migrateRemote` + gen) · Task 4 (`initDatabase`) |
| §3.1 user/device/setting · §7 PIN ตรวจในเครื่อง · §5 ล็อกอิน/ล็อก 10 นาที | Task 6 |
| §4.8 ห้ามขายถ้าไม่มีกะ · business_date | Task 7 (`openShift`) · Task 10 (`NO_OPEN_SHIFT`) |
| §5 หน้าขาย: แท็บหมวด + "ขายดี" (D27) · ปุ่ม ≥ 64 px · popup 16 oz/50% · ตะกร้า · ปุ่มเงินสด/QR | Task 8, 9 |
| §4.1 ราคา ณ เวลาขาย · ห้ามขายของไม่มีราคา · ยอด/ส่วนลด/ทอนเงิน | Task 2 (`planSale`, `requirePrice`, `cashChangeSatang`) · Task 10 |
| §1.2/§3.4 ส่วนลดจำนวนเงินระดับบิล + เหตุผล | Task 8 (reducer) · Task 9 (dialog) · Task 10 (`discount` + `DISCOUNT_APPLIED`) |
| §5 ชำระเงินสด (ปุ่มเร็ว + เงินทอนตัวใหญ่) · QR Tag 29 ใส่ยอด ยืนยันเอง | Task 2 (`promptPayPayload`, `quickTenderOptions`) · Task 11 · Task 12 |
| §4.7 เลขที่บิลไม่มีช่องว่าง/ไม่ซ้ำ · เลขคิวรีเซ็ตรายวัน | Task 2 (`nextReceiptNo`) · Task 10 (เทสต์ concurrent, rollback ไม่กินเลข, ข้ามวัน) |
...
| §4.9 + D38 โซ่แฮชต่อเครื่อง ครอบ order_id/seq/device_id | Task 5 · ตรวจด้วย `verifyChain` ใน Task 5, 10, 13 |
| §6.1 1 transaction = ธุรกรรม + movement + event + outbox | Task 5 (`enqueueOutbox`) · Task 7, 10, 13 |
| §5 หลังชำระ: เลขคิวตัวใหญ่ + รายการ · ปิดเอง 5 วิ | Task 12 (`DoneScreen`) |
| §5 ประวัติบิล · เปิดดู · event timeline | Task 11, 14 |
| §4.3 + D18/D36/D39 ยกเลิก: เหตุผล + owner PIN + ทำแล้วหรือยัง · VOID_RETURN · waste marker · VOID_REFUND · QR ใส่อ้างอิงโอนคืน · บิลยังอยู่ | Task 13, 14 |
| §12 "ยังไม่ส่ง N รายการ" บนจอตลอด | Task 4 (`outboxPending`) · Task 9 (`pending-sync`) |
| §8 E2E ขาย → ชำระ → ตัดสต็อก → ยกเลิก · ออฟไลน์ขายต่อ | Task 12, 14, 15 (สต็อกตรวจละเอียดใน integration Task 10, 13) |

**ไม่อยู่ในแผนนี้ (ตั้งใจ):** ปิดกะ/นับธนบัตร/X/Z และปุ่มสำรองไฟล์ → แผน 3b (รอคำตอบ Q3-17) · ส่ง outbox/sync + ส่วน "ปิดกะ → sync ถึงเซิร์ฟเวอร์" ของ e2e §8 → แผน 5, 8 · `order_payment_intent`, สถานะ `ready`/`picked_up` → แผน 6 (LINE) · VAT เปิดใช้ → config ปิดไว้ (`VAT_OFF`) · เปิดกะด่วนแบบเงินทอน 0 (§4.8) ทำได้แล้วเพราะช่องเงินทอนรับ 0 · staff role เตรียมไว้ใน `requireOwnerPin` แต่ยังไม่มีหน้าจัดการผู้ใช้ (แผนจัดการข้อมูลหลัก)

**2. Placeholder scan:** ค้น `TBD|TODO|implement later|similar to` แล้วไม่พบ · ช่องที่ต้องกรอกเฉพาะไฟล์บันทึกผล (ผล spike, บันทึกการทำแผน) ซึ่งเป็นแบบฟอร์มให้กรอกค่าจริง · จุดที่รอเจ้าของใช้รูป `(รอคำตอบ Q3-n)` เท่านั้น

**3. Type consistency:** `PosApi` มีเมธอด 12 ตัว และ `POS_API_METHODS` ถูกตรวจด้วย type `POS_API_METHODS_COMPLETE` (ลืมใส่ = typecheck ตก) · ชื่อที่ใช้ข้าม task: `RemoteDb`, `migrateRemote`, `oo1Callback`, `nodeSqliteCallback`, `deterministicIds`, `applySeedRemote`, `loadCatalogRemote`, `initDatabase`, `createPosApi`, `ApiDeps`, `PosError`/`posErrorCode`, `enqueueOutbox`, `appendOrderEvents`, `loadDeviceChain`, `requireDevice`, `currentOpenShift`, `requireStoreChannelId`, `loadSaleContext`, `insertMovements`, `getOrder`, `requireOwnerPin`, `openReadyApi`, `sellSku`, `TEST_SETUP`, `PINS`, `TEST_PIN_COST` — ตรวจตรงกันทุกจุด · `EventCore` ของ domain หลังแก้ D38 ถือว่ามี `seq` และ `deviceId` (Task 1 Step 1 ตรวจ และให้ใช้ชื่อจริงถ้าต่าง)

**4. ความเสี่ยงที่รู้อยู่:** `sqlite-proxy` ส่ง `begin/commit` ผ่าน callback — ปลอดภัยเพราะทุกเมธอดวิ่งผ่าน `createSerialQueue` (เทสต์ concurrent ใน Task 10) · `opfs-sahpool` เปิดได้ทีละแท็บ → `DbErrorScreen` บอกให้ปิดแท็บอื่น · `node:sqlite` ยังเป็น experimental ใน Node 22 (เตือนได้ ไม่ใช่ error) · ต้องตรวจชื่อคอลัมน์จริงของแผน 2 ฉบับแก้ D36–D38 ก่อนเริ่ม (Task 1 Step 1)
