...
:root {
  ...
  /* รอ Q3-23: matcha ใช้ในคลื่น (wave.svg) — README โลโก้ให้ matcha แค่ผิวน้ำชาในแก้วเท่านั้น · selected/muted/line/surface เป็นโทเค็นเพิ่มนอก D44 */
...
```
แนบ `brand.png` ในรายงาน แจ้งเจ้าของตรวจว่าแถบบนเขียวเข้ม ตัวอักษรครีม โลโก้ไม่มีสโลแกน ใต้แถบมีคลื่นส้ม/เขียว · ไม่มีตัวอักษรสีส้มหรือสีมัทฉะที่ใดในแอป (`grep -rn "ed7a13\|7ea05f" -i apps/pos/src` ต้องเจอเฉพาะบรรทัด token ใน `styles.css`)
...
// รอ Q3-21: ยังไม่มี rate limit / lockout ตอนใส่ PIN ผิดซ้ำ ๆ (ทั้ง login และ requireOwnerPin ด้านล่างเรียกผ่านฟังก์ชันนี้)
...
export function SessionProvider({ children }: { children: ReactNode }): JSX.Element {
  ...
  const lock = useCallback(() => setUser(null), []) // CartProvider is a separate context, so locking keeps the cart as-is (รอ Q3-24)
...
describe('cartReducer', () => {
  ...
  it('accepts a discount only with a reason, above zero and not above subtotal (D48 Q3-6)', () => {
    ...
    // a bill must stay above 0: payment.amount_satang > 0 is a DB CHECK (D47 item 7) · Q3-20
...
export async function loadMenu(db: RemoteDb, deps: ApiDeps): Promise<MenuDto> {
  ...
  // Window is calendar days, not business days with sales — see owner question Q3-25 (รอ Q3-25 ถ้าคำตอบเปลี่ยนเป็นวันทำการ).
...
export function cartReducer(state: CartState, action: CartAction): CartState {
  switch (action.type) {
    ...
    case 'setDiscount': {
      ...
      if (!Number.isSafeInteger(amountSatang) || amountSatang <= 0 || amountSatang >= cartSubtotalSatang(state) || reason === '') return state // total stays > 0 (Q3-20)
...
export function DiscountDialog({ onClose }: { onClose: () => void }): JSX.Element {
  ...
  const apply = (): void => {
    ...
    if (amountSatang >= cartSubtotalSatang(state)) return setError(TH.errDiscountTooBig) // total stays > 0 (Q3-20)
...
describe('commitSale', () => {
  ...
  it('failures roll back everything and do not consume a receipt number', async () => {
    ...
    await expect(cashSale(t, 'Latte-16oz', 1, 10_000, { discount: { amountSatang: 5000, reason: 'ฟรี' } })).rejects.toThrow(/^DISCOUNT_TOO_BIG: /) // 0-baht bill (Q3-20)
  ...
  // M6: window is the last 7 calendar days anchored on the open shift's business date (รอ Q3-25 — ถ้าคำตอบเปลี่ยนเป็น
  // "7 วันทำการที่มียอดขาย" ต้อง group by business_date ที่มีแถวใน order แทนช่วงปฏิทินตรง ๆ)
  it('best sellers rank products by cups sold in the last 7 calendar days, anchored on the open shift (D48 Q3-9, รอ Q3-25)', async () => {
    const t = await openReadyApi()
...
export async function commitSale(db: RemoteDb, deps: ApiDeps, input: CommitSaleInput): Promise<CommitSaleResult> {
  ...
  return db.transaction(async (tx) => {
    ...
    // No 0-baht bills: payment.amount_satang > 0 is a DB CHECK (D47 item 7). Free drinks wait for the owner (Q3-20).
...
export type CommitSaleInput = {
  ...
  // รอ Q3-27 (ตรง I-7 ของ plan-3-review.md): commitSale re-prices from the DB at `now` without checking it
  // against the total the cart/QR showed — a price change between menu load and payment is not caught here.
}
...
export function VoidDialog({ order, onClose }: { order: OrderDetailDto; onClose: () => void }): JSX.Element {
  ...
  const [approverId, setApproverId] = useState<string | null>(user?.role === 'owner' ? user.id : null) // preselects the signed-in owner (รอ Q3-22)
  const [error, setError] = useState<string | null>(null)
