...
- **กฎของ schema ที่ DB บังคับ (D47) — โค้ดต้องเคารพ ไม่ใช่พึ่ง DB จับ:** CHECK constraint ที่มีจริงในแผน 2 (ตรวจ 18 ก.ย.): `cash_movement` kind `VOID_REFUND` ⇔ มี `order_id` และ `amount_satang > 0` · `order` subtotal/discount/total ≥ 0 และ discount ≤ subtotal · `order_line.qty > 0` · `payment.amount_satang > 0` · `stock_movement.qty_milli ≠ 0` → เขียนแถวเงินเฉพาะเมื่อยอด > 0 · ใส่ `order_id` ทุกแถว `VOID_REFUND` · **ไม่มีบิล 0 บาท** ส่วนลดต้องน้อยกว่ายอดรวม (เพราะ `payment > 0` — คำถามใหม่ Q3-20 ค่าแนะนำนี้ใช้ในแผนไปก่อน) · `unique(device_id, business_date, queue_no)` → เลขคิวคำนวณจาก (เครื่อง, business_date) ใน transaction เดียวกับการ insert · partial unique เปิดกะได้หนึ่งกะต่อเครื่อง → ตรวจ `SHIFT_ALREADY_OPEN` ก่อน insert (DB เป็นด่านสุดท้าย) · ตาราง R ทุกตารางมี `version` + `updated_at` → แถว R ที่แอปสร้าง (`device`, `user`, `setting`) ใส่ `version: 1`, `updatedAt: now` · `setting` คีย์ `(key, effective_from)` → อ่านค่าที่ `effective_from ≤ ตอนนี้` ล่าสุด · ยกเลิกบิลได้เฉพาะบิลใน **กะที่เปิดอยู่** (D47 ข้อ 2) · ชื่อคอลัมน์/CHECK ที่แท้จริงดูจาก `packages/db-schema/src/sqlite/*` — ถ้าต่างจากแผนนี้ ให้ใช้ของจริงและจดในบันทึกการทำแผน
...
- **แบรนด์ (D44):** สีทั้งหมดอยู่ใน token ของ `styles.css` เท่านั้น · Deep Forest `#1E3B22` = ตัวอักษรหลัก/แถบบน/ปุ่มหลัก · Cream `#F6F3E8` = พื้น และตัวอักษรบนแถบเขียว · Matcha `#7EA05F` = **เฉพาะผิวน้ำชาในแก้วของโลโก้** ไม่ใช้ในแอป/คลื่น/ตัวอักษร (D50 Q3-23) · ส้มป้าย `#ED7A13` = คลื่น/เส้นตกแต่งเท่านั้น · คลื่นใต้แถบบน = ส้ม + Deep Forest `#1E3B22` / เขียวเข้มอ่อนลง `#35613B` · สีกลางเพิ่ม `#FFFDF6` `#E4ECD9` `#5C6B5E` `#D6D3C4` อนุมัติแล้ว (D50) · พื้นแอป Cream `#F6F3E8` · `#FFFADD` ใช้กับป้ายร้านเท่านั้น ไม่ใช้ในแอป (ส้มบนครีม 2.56:1 ใช้เป็นตัวอักษรไม่ได้แม้ตัวใหญ่ · ส้มบนเขียวเข้ม 4.34:1 ใช้ได้เฉพาะตัวอักษรใหญ่ ≥ 24 px) · ข้อความทุกคู่สีต้อง ≥ 4.5:1 (Deep Forest บน Cream 11.09:1 · Cream บน Deep Forest 11.09:1) · ไอคอน PWA = `appicon_cream` · โลโก้ในแถบบนใช้ `DA-YO_reversed.svg` ที่ตัดสโลแกนออก (โลโก้เล็กกว่า ~25 มม.)
...
- เครื่องพัฒนาเป็น Windows 11 + Git Bash · Node 22.16 ขึ้นไป (`node -v`)
...
| S8 | argon2idAsync t=2 m=19456 × 2 พร้อมกัน (M9: ตอนตั้งค่าแฮช PIN เจ้าของ 2 คนพร้อมกัน แบบเดียวกับแอปจริงที่ใช้ `argon2idAsync`) | ✅ | ≤ 2000 ms |
...
| I4 | เวลา seed ครั้งแรก (`applySeedSqlite`, M9) | ข้อมูล | บันทึกเวลา — ไม่นับใน S7 |
...
git checkout main
git log --oneline -1                     # 4bee0c1 หรือใหม่กว่า (แผน 2 merge แล้ว)
git checkout -b plan-3-pos-sell
```
...
Run: `pnpm --filter @dayo/db-schema exec vitest run browser-migrate`
...
Expected: PASS ทั้งหมด (รวม 6 เทสต์ใหม่ และเทสต์เดิมของแผน 2) · มี `ExperimentalWarning: SQLite is an experimental feature` ได้ ไม่ใช่ error · ถ้า `No such built-in module: node:sqlite` หรือ `setReturnArrays is not a function` → Node เก่ากว่า 22.16 (M1) ให้ `nvm install 22 && nvm use 22` · ถ้า typecheck บ่นว่า `DatabaseSync` แปลงเป็น `NodeSqliteLike` ไม่ได้ ให้คง cast `as unknown as NodeSqliteLike` ไว้ (ใส่แล้ว) · ถ้า `dialect.migrate` ไม่มีใน drizzle เวอร์ชันที่ติดตั้ง ให้หยุดและรายงาน (ห้ามเขียน runner เองที่ใช้ตาราง bookkeeping อื่น)
...
```
Expected: ไม่มีบรรทัดใด (ยกเว้นคอมเมนต์ใน `migrate.ts` ที่พูดถึง sql-js — ถ้าเจอบรรทัด import จริงถือว่าผิด) · M21: `grep -v "^.*import type"` กรองทิ้งทั้งบรรทัดที่มี `import type` แม้เป็นบรรทัดผสม ยอมรับได้ แต่ต้องแน่ใจว่า SQL ข้อความของ `SQLITE_MIGRATIONS` ใน `sqlite-migrations.gen.ts` ไม่มีคำว่า `node:` อยู่เลย (ไม่ใช่แค่ import statement)
...
`apps/pos-spike/public/manifest.webmanifest` (I-5: without this the spike can only be added as a Chrome "shortcut", and S9's persistence heuristic differs for a shortcut vs. a real installed PWA)
```json
{"name":"DA-YO spike","short_name":"spike","start_url":"/","display":"standalone","icons":[{"src":"/icon.svg","sizes":"any","type":"image/svg+xml"}]}
```

`apps/pos-spike/public/sw.js` (empty fetch handler — enough for Chrome to treat the page as installable)
```js
self.addEventListener('fetch', () => {})
```

`apps/pos-spike/public/icon.svg` (สี่เหลี่ยมจัตุรัส SVG ใดก็ได้ ใช้เป็นไอคอนชั่วคราวของ spike เท่านั้น)
```svg
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect width="100" height="100" fill="#1E3B22"/></svg>
```
...
import { product, syncState } from '@dayo/db-schema/sqlite'
...
async function run(): Promise<SpikeReport> {
  ...
  // I4 (M9): first-run seed time — not counted in S7's cold start, but it runs once on the real first boot.
  // applySeedSqlite is one transaction and not idempotent (like apps/pos/src/db/init.ts's isSeeded), so on a
  // reload — this whole run() re-executes — it must run at most once per DB file, or the second insert throws.
  const alreadySeeded = ((await db.select({ c: count() }).from(product).get())?.c ?? 0) > 0
  const tSeed = performance.now()
  if (!alreadySeeded) await applySeedSqlite(db, seedToRows(parseSeed(seedJson), canonicalSeedOpts()))
  checks.push({ id: 'I4', title: 'first-run seed (applySeedSqlite)', required: false, pass: true, detail: alreadySeeded ? 'already seeded (reload)' : `${since(tSeed)} ms` })
...
- [ ] **Step 9: รัน spike บนแท็บเล็ต Android + Chrome (D46) ผ่าน https ด้วย Cloudflare Quick Tunnel (D48 Q3-2)** ⛔ HAND-OFF เจ้าของร้าน

> **ขั้นนี้เจ้าของทำเองบนแท็บเล็ต — agent ห้ามทำแทนหรือกรอกผลแทน.** Agent: commit Step 1–8, เตรียมไฟล์ `ผลspike.md` จากแม่แบบโดยกรอกเฉพาะแถว "ผล Chromium บนเครื่อง dev (Step 8)" แล้ว **หยุด** แจ้งเจ้าของพร้อมคำสั่งใน Step 9 · ทำต่อ Step 10–11 เมื่อเจ้าของส่งค่าที่วัดได้กลับมาเท่านั้น
...
```
เปิด Git Bash หน้าต่างใหม่ก่อนเรียก `cloudflared` (M19: `winget install` แก้ PATH เฉพาะ shell ใหม่ ไม่มีผลกับหน้าต่างที่เปิดอยู่)
```bash
...
> **Keep one cloudflared process running from item 1 to item 5.** ถ้า URL เปลี่ยน แปลว่าเป็น origin ใหม่: เริ่มนับใหม่จากข้อ 1 และห้ามบันทึกว่าล้มเหลว สำหรับ S10 (รีสตาร์ตแท็บเล็ต) ให้ใช้ `adb reverse tcp:4180 tcp:4180` + `http://localhost:4180` แทน — origin นั้นคงที่ และ localhost เป็น secure context อยู่แล้ว บันทึกในผล spike ว่าแต่ละข้อใช้วิธีไหน
...
ใช้ skill `committing-code` แล้ว:
```bash
git add .gitignore apps/pos-spike/package.json apps/pos-spike/tsconfig.json apps/pos-spike/vite.config.ts apps/pos-spike/playwright.config.ts apps/pos-spike/index.html apps/pos-spike/public/manifest.webmanifest apps/pos-spike/public/sw.js apps/pos-spike/public/icon.svg apps/pos-spike/src/types.ts apps/pos-spike/src/worker.ts apps/pos-spike/src/main.ts apps/pos-spike/e2e/spike.spec.ts pnpm-lock.yaml docs/superpowers/plans/2026-09-17-03-ผลspike.md
...
- Create (D50 Q3-23 — เพิ่มหลัง commit Task 3 เป็น commit แยก): `apps/pos/test/brand.test.ts`
...
| Matcha `#7EA05F` | — | **ไม่ใช้ในแอป** — อยู่เฉพาะผิวน้ำชาในแก้วของโลโก้ (README โลโก้, D50 Q3-23) |
| `#35613B` (Deep Forest อ่อนลง) บน Deep Forest | — | แถบล่างของคลื่นเท่านั้น (ตกแต่ง ไม่ใช่ตัวอักษร · D50 Q3-23) |

**Interfaces:**
- Produces:
  - `ui/BrandBar.tsx`: `BrandBar()` แถบบนเขียวเข้มสูง 56 px + โลโก้ `brand/logo-header.svg` (testid `brand-logo`) + ชื่อแอปสีครีม + แถบคลื่นส้ม/เขียวเข้ม (ไม่มี Matcha — D50) — วางใน root route จึงอยู่ทุกหน้า
...
```
M11: `@testing-library/dom` (required peer of `@testing-library/react@16`) and `workbox-build`/`workbox-window` (required peers of `vite-plugin-pwa@1.3.0`) are today auto-installed by pnpm but not pinned in this file — pin them before Step 1's `pnpm install`:
```bash
pnpm view @testing-library/dom version
pnpm view workbox-build version
pnpm view workbox-window version
```
แทนค่า `"PIN"` ทั้ง 3 ช่องด้วยเวอร์ชันจริงที่ได้ (รูปแบบเดียวกับ T27 ที่ตรวจด้วย `pnpm view` วันที่ 2026-09-17) ก่อนรัน `pnpm install`
...
`apps/pos/public/brand/wave.svg` (แถบคลื่นใต้แถบบน ตามสไตล์ป้ายร้าน: ส้มบน เขียวเข้มอ่อนลง `#35613B` ล่าง บนพื้น Deep Forest · **ไม่ใช้ Matcha** — D50 Q3-23 · ต่อกันได้ทุก 120 หน่วย)
```svg
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 12" width="120" height="12"><title>DA-YO wave</title><rect width="120" height="12" fill="#1E3B22"/><path d="M0 4 Q15 0 30 4 T60 4 T90 4 T120 4 V8 H0 Z" fill="#ED7A13"/><path d="M0 8 Q15 4 30 8 T60 8 T90 8 T120 8 V12 H0 Z" fill="#35613B"/></svg>
```

⛔ HAND-OFF เจ้าของร้าน (I-10): agent ไม่ตัดสินผลด้วยตาเอง — ให้ถ่ายภาพแล้วหยุดถามเจ้าของ:
```bash
pnpm --filter @dayo/pos exec playwright screenshot apps/pos/public/brand/logo-header.svg logo-header.png
```
แนบ `logo-header.png` ในรายงาน แจ้งเจ้าของตรวจว่าเห็นตัวการ์ตูน + "だよ" + "DA-YO" ครบ และ **ไม่มี** เศษของ "GOOD DRINK · GOOD DAY" ที่ขอบล่าง · ถ้ามีเศษ ให้ลดความสูงลงทีละ 50 (ทั้ง `viewBox` และ `height` ÷ 10) จนหาย แล้วถ่ายซ้ำ
...
```css
:root {
  /* DA-YO brand (D44, D50 Q3-23). Orange is decoration only — never text (contrast 2.56:1 on cream).
     Matcha lives only in the logo's tea; the wave (wave.svg) is orange on Deep Forest (#1E3B22 / #35613B).
     The signage yellow is for shop signage only, never the app. selected/muted/line/surface are approved extra neutrals (D50). */
  ...
  --qr-bg: #ffffff;        /* M5: PromptPay QR quiet zone — scanners need a plain white background */
  --backdrop: rgb(30 59 34 / 0.45); /* M5: dialog backdrop */
...
.dialog-backdrop { position: fixed; inset: 0; background: var(--backdrop); display: grid; place-items: center; padding: 16px; }
...
.qr img { width: min(80vw, 560px); height: auto; image-rendering: pixelated; background: var(--qr-bg); }
...
```
(ส้มปรากฏเฉพาะใน `wave.svg` และเส้นขอบบนของ dialog ซึ่งเป็นการตกแต่ง · ไม่มี Matcha ในแอป (D50 Q3-23) ไม่ได้สื่อสถานะ · สถานะเลือก/โฟกัสใช้ Deep Forest ทั้งหมด · พื้นหลัง QR เป็นขาวเพื่อให้แอปธนาคารสแกนได้ง่าย · เลขคิวใช้ Deep Forest ไม่ใช้ส้ม เพราะส้มบนครีมไม่ผ่าน 3:1 แม้ตัวใหญ่)

`apps/pos/test/brand.test.ts` (D50 Q3-23 — กันไม่ให้ Matcha/`#FFFADD` หลุดเข้าแอปอีก)
```ts
import { readFileSync } from 'node:fs'
import { fileURLToPath } from 'node:url'
import { describe, expect, it } from 'vitest'
...
```
Expected: format test + brand test PASS · typecheck สะอาด · e2e `1 passed` · `apps/pos/dist/sw.js` ถูกสร้างตอน build · `grep -c "icon-192.png" apps/pos/dist/manifest.webmanifest` ได้ `1`

⛔ HAND-OFF เจ้าของร้าน (I-10): agent ไม่ตัดสินผลด้วยตาเอง — รัน `pnpm --filter @dayo/pos preview` แล้วถ่ายภาพ:
```bash
pnpm --filter @dayo/pos exec playwright screenshot --viewport-size=1280,200 http://localhost:4173 brand.png
```
แนบ `brand.png` ในรายงาน แจ้งเจ้าของตรวจว่าแถบบนเขียวเข้ม ตัวอักษรครีม โลโก้ไม่มีสโลแกน ใต้แถบมีคลื่นส้ม/เขียวเข้ม (ไม่มีสีมัทฉะ) · ไม่มีตัวอักษรสีส้มที่ใดในแอป (`grep -rn "ed7a13" -i apps/pos/src` ต้องเจอเฉพาะบรรทัด token ใน `styles.css` · `grep -rn "7ea05f\|fffadd" -i apps/pos/src apps/pos/public/brand/wave.svg` ต้องไม่เจอ — D50)
...
- Produces:
  ...
  - `api/types.ts`: `UserDto = { id; displayName; role: UserRole }` · `DeviceDto = { id; name; receiptPrefix }` · `ShiftDto = { id; businessDate; openedAt; openedBy; openingFloatSatang }` · `BootstrapState = { needsSetup; device: DeviceDto | null; users: UserDto[]; openShift: ShiftDto | null; pendingSyncItems: number }` · `interface PosApi { bootstrap(): Promise<BootstrapState> }` · `POS_API_METHODS` · `PIN_RE`
  ...
  - `api/bootstrap.ts`: `LOCAL_DEVICE_KEY` · `localDeviceId(db)` · `requireDevice(db): Promise<DeviceDto>` (throw `NEEDS_SETUP`) · `currentOpenShift(db, deviceId): Promise<ShiftDto | null>` · `listActiveUsers(db)` · `countPendingSyncItems(db)` (นับ **บิล** ที่ยังมีแถว `status = 'pending'` บิลละ 1 + รายการอื่นที่ไม่ใช่บิล เช่น กะ รายการละ 1 — ไม่นับแถว outbox · D50 Q3-26 · เทสต์อยู่ Task 5 Step 5 เพราะต้องใช้ `enqueueOutbox`) · `bootstrap(db)`
  ...
  - `ui/errors.ts`: `errorMessage(e: unknown): string` · `errorDetail(e: unknown): string` (M10 — raw text for an optional `<details>`, never the primary message)
...
Run: `pnpm --filter @dayo/db-schema exec vitest run browser-seed`
...
- Create (D50 Q3-26 — เพิ่มหลัง commit Task 5 เป็น commit แยก, Step 5): `apps/pos/test/pending-sync.test.ts` · Modify: `apps/pos/src/api/bootstrap.ts`, `src/api/types.ts`, `src/screens/IndexRedirect.tsx`, `test/init.test.ts` (`countPendingOutbox`/`outboxPending` → `countPendingSyncItems`/`pendingSyncItems`)
...
Run: `pnpm --filter @dayo/pos exec vitest run events`
...
- [ ] **Step 5: ป้าย "ยังไม่ส่ง N รายการ" นับบิล ไม่นับแถว outbox (D50 Q3-26)**

บิลหนึ่งใบเขียน outbox 20+ แถว (order, order_line, discount, payment, order_event ×4, stock_movement ต่อวัตถุดิบ) ตัวเลขแถวทำให้ดูเหมือนมีปัญหา → นับ **บิล** (order id ที่ยังมีแถว pending) บิลละ 1 · แถวที่ไม่ใช่ของบิล (กะ, เงินเข้า/ออก, stock movement ที่ไม่ใช่ `refType = 'order'`) นับ **ต่อ id** รายการละ 1 เพื่อไม่ให้ป้ายขึ้น 0 ทั้งที่ยังมีข้อมูลค้างส่ง · แถว `order` ที่ส่งซ้ำตอน void (`keySuffix 'voided'`) และ `cash_movement` `VOID_REFUND` นับรวมกับบิลเดิม · กะที่มีทั้งแถวเปิดและปิดนับ 1

`apps/pos/test/pending-sync.test.ts` (เขียนก่อน — ต้องตกเพราะยังไม่มี `countPendingSyncItems`)
```ts
import { eq } from 'drizzle-orm'
import { describe, expect, it } from 'vitest'
import * as s from '@dayo/db-schema/sqlite'
...
แล้วแก้ `apps/pos/src/api/bootstrap.ts` ให้เป็นโค้ด `countPendingSyncItems` ที่อยู่ใน Task 4 Step 4 · เปลี่ยนชื่อฟิลด์ `BootstrapState.outboxPending` → `pendingSyncItems` ใน `types.ts`, `IndexRedirect.tsx`, `test/init.test.ts`

Run: `pnpm --filter @dayo/pos test && pnpm --filter @dayo/pos typecheck && pnpm --filter @dayo/pos e2e`
Expected: PASS

ใช้ skill `committing-code` แล้ว:
```bash
git add apps/pos/src/api/bootstrap.ts apps/pos/src/api/types.ts apps/pos/src/screens/IndexRedirect.tsx apps/pos/test/init.test.ts apps/pos/test/pending-sync.test.ts
git commit -m "fix(pos): count pending sync by bill instead of outbox row"
```
...
**Files:**
- Create: `apps/pos/src/lib/pin.ts`, `apps/pos/src/api/setup.ts`, `apps/pos/src/api/auth.ts`, `apps/pos/src/api/pin-guard.ts`, `apps/pos/test/setup-auth.test.ts`
...
- Modify: `apps/pos/src/api/types.ts`, `apps/pos/src/api/pos-api.ts`, `apps/pos/src/app/providers.tsx`, `apps/pos/src/router.tsx`, `apps/pos/src/api/errors.ts`, `apps/pos/src/ui/errors.ts`, `apps/pos/src/ui/th.ts` (D50 Q3-21: `PIN_LOCKED`)

**ล็อก PIN เมื่อผิดซ้ำ (D50 Q3-21):**
- ใช้ทั้งตอนล็อกอินและตอนอนุมัติด้วย PIN เจ้าของ (`requireOwnerPin` เรียก `login`) → **ตัวนับเดียวกัน ต่อผู้ใช้หนึ่งคน** (ผู้ใช้ A ถูกล็อก ผู้ใช้ B ยังล็อกอินได้ · สลับไปเดา PIN คนอื่นไม่ได้ช่วยเพราะแต่ละคนนับแยก)
- ผิด **5 ครั้งติด** → รอ **30 วินาที** · ผิดอีกครั้งหลังพ้นเวลา (ยังไม่เคยสำเร็จ) รอเป็นเท่าตัว 60, 120, 240, 480 วินาที แล้ว **สูงสุด 15 นาที** (900 วินาที) ทุกครั้งต่อจากนั้น · ใส่ถูกหนึ่งครั้ง → ล้างตัวนับ
- ระหว่างล็อก ระบบ **ไม่ตรวจ PIN เลย** (PIN ถูกก็ไม่ผ่าน · ไม่นับเป็นครั้งผิด · ไม่รัน argon2) และตอบ `PIN_LOCKED: <วินาทีที่เหลือ ปัดขึ้น>` · ครั้งที่ทำให้ล็อกตอบ `PIN_LOCKED` ทันที (ไม่ใช่ `PIN_WRONG`) เพื่อให้จอบอกเวลารอเลย
- **ที่เก็บตัวนับ:** ตาราง `sync_state` (key/value ในเครื่องเท่านั้น ไม่ sync — ที่เดียวกับ `local.device_id`) คีย์ `pin_guard.<userId>` ค่า JSON `{ fails, lockedUntil }` จึง **อยู่รอดเมื่อรีโหลดหน้า** · ไม่ใช้ `setting` เพราะเป็นตาราง R ที่เซิร์ฟเวอร์เป็นเจ้าของและมี `version`/`effective_from` (D47 ข้อ 8–9) · ไม่ใช้ `audit_log` เป็นที่เก็บสถานะเพราะต้องไล่นับย้อนหลังทุกครั้ง — ใช้เป็นบันทึกอย่างเดียว
- **audit ทุกครั้ง:** ผิดแต่ละครั้ง → `audit_log` `entity 'user'`, `action 'pin_fail'`, `after_json { fails }` · ครั้งที่ล็อก → เพิ่ม `action 'pin_lockout'`, `after_json { fails, lockSeconds, lockedUntil }` · `actor_user_id = null` (ไม่รู้ว่าใครกด) · **ไม่เก็บ PIN ที่กด**
- เวลามาจาก `deps.now()` (เทสต์ใช้ `testClock().advanceMs`) · จอแสดง `TH.errPinLockedFor(n)` = "ใส่ PIN ผิดหลายครั้ง — ลองใหม่ใน N วินาที" ผ่าน `errorMessage` ทั้งหน้า login และหน้าต่างยกเลิกบิล (Task 14) โดยไม่ต้องแก้หน้าจอเพิ่ม · ตัวเลขเป็นค่าตอนที่กด (ไม่นับถอยหลังสด) — กดอีกครั้งระหว่างล็อกจะได้เวลาที่เหลือล่าสุด และไม่นับเป็นครั้งผิด
...
- Produces:
  ...
  - `api/auth.ts`: `login(db, deps, userId, pin): Promise<UserDto>` (throw `PIN_WRONG` / `PIN_LOCKED`) · `requireOwnerPin(db, deps, userId, pin): Promise<UserDto>` (throw `PIN_WRONG` / `PIN_LOCKED` / `NOT_OWNER`)
  - `api/pin-guard.ts` (D50 Q3-21): `PIN_FREE_TRIES = 5` · `PIN_LOCK_BASE_MS = 30_000` · `PIN_LOCK_MAX_MS = 900_000` · `pinLockMs(fails): number` · `type PinGuardState = { fails: number; lockedUntil: string | null }` · `readPinGuard(db, userId)` · `assertNotLocked(state, nowIso)` (throw `PIN_LOCKED`) · `recordPinFailure(db, deps, userId, state): Promise<PinGuardState>` · `clearPinGuard(db, userId)`
  - `api/errors.ts` เพิ่ม `'PIN_LOCKED'` ใน `PosErrorCode` (detail = วินาทีที่เหลือ) · `ui/th.ts` เพิ่ม `errPinLocked`, `errPinLockedFor(seconds)` · `ui/errors.ts` เพิ่ม `PIN_LOCKED` ใน `MESSAGES` และแสดงวินาทีจาก detail
...
Run: `pnpm --filter @dayo/pos test`
Expected: FAIL — resolve `../src/lib/pin`, `../src/api/auth`, `../src/api/pin-guard`, `./session`, `./PinPad` ไม่ได้
...
`apps/pos/src/api/pin-guard.ts` (D50 Q3-21)
```ts
import { eq } from 'drizzle-orm'
import type { RemoteDb } from '@dayo/db-schema/browser'
import * as s from '@dayo/db-schema/sqlite'
...
แก้ `apps/pos/src/api/errors.ts` — เพิ่มใน `PosErrorCode` ต่อจาก `'PIN_WRONG'`:
```ts
  | 'PIN_LOCKED' // D50 Q3-21 — detail is the whole seconds left
```

แก้ `apps/pos/src/ui/th.ts` — เพิ่มต่อจาก `errPinWrong`:
```ts
...
แก้ `apps/pos/src/ui/errors.ts` — เพิ่มใน `MESSAGES` ต่อจาก `PIN_WRONG` และให้ `errorMessage` แสดงวินาทีจาก detail (แทรกก่อนบรรทัด `return` สุดท้าย):
```ts
...
Run: `pnpm --filter @dayo/pos test && pnpm --filter @dayo/pos typecheck`
Expected: PASS ทั้งหมด (setup-auth รวม PIN lockout, session, PinPad และเทสต์เดิม)

- [ ] **Step 5: Commit**

ใช้ skill `committing-code` แล้ว:
```bash
git add apps/pos/src/lib/pin.ts apps/pos/src/api/setup.ts apps/pos/src/api/auth.ts apps/pos/src/api/pin-guard.ts apps/pos/src/api/errors.ts apps/pos/src/ui/errors.ts apps/pos/src/ui/th.ts apps/pos/test/setup-auth.test.ts apps/pos/src/app/session.tsx apps/pos/src/app/session.test.ts apps/pos/src/screens/PinPad.tsx apps/pos/src/screens/PinPad.test.tsx apps/pos/src/screens/SetupScreen.tsx apps/pos/src/screens/LoginScreen.tsx apps/pos/src/api/types.ts apps/pos/src/api/pos-api.ts apps/pos/src/app/providers.tsx apps/pos/src/router.tsx
git commit -m "feat(pos): first-run device setup, argon2id PINs, login with PIN lockout and idle auto-lock"
```
...
Run: `pnpm --filter @dayo/pos exec vitest run shift`
...
```
และแก้ `/sell` ให้มี guard ตั้งแต่ตอนนี้ (ไม่งั้นเทสต์ reload ด้านล่างจะไม่มีความหมาย จนกว่า Task 9 จะเปลี่ยน placeholder เป็น `<SellScreen />`):
```tsx
...
pnpm --filter @dayo/pos e2e
```
Expected: PASS ทั้งหมด · e2e `2 passed` (หน้า `/sell` ยังว่างจนถึง Task 9 — ถูกต้อง)
...
**Interfaces:**
- Consumes: `priceFor`, `computeTotals`, `VAT_OFF`, `Totals` (domain) · `bangkokDate`, `addDaysToDate` (Task 4) · `localDeviceId`, `currentOpenShift` (Task 4, M6: anchor the best-seller window on the open shift's business date, falling back to the wall clock before setup)
- Produces:
  ...
  - `state/cart.ts`: `CartLine = { key; variantId; sweetnessId; productName; sizeName; sweetnessName; unitPriceSatang; qty }` · `NewCartLine = Omit<CartLine, 'key' | 'qty'>` · `CartDiscount = { amountSatang; reason }` · `CartState = { orderId; lines: CartLine[]; discount: CartDiscount | null }` · `CartAction` (`add` `inc` `dec` `remove` `setDiscount` `clearDiscount` `reprice` `reset` — `reprice` ใช้ตอน `PRICE_CHANGED` ใน Task 12, D50 Q3-27) · `lineKey(variantId, sweetnessId)` · `emptyCart(orderId)` · `cartSubtotalSatang(state)` · `cartTotals(state): Totals` · `toSaleLines(state): { variantId; sweetnessId; qty }[]` · `cartReducer(state, action): CartState`
...
- Modify: `apps/pos/src/api/types.ts`, `apps/pos/src/api/pos-api.ts`, `apps/pos/src/api/errors.ts`, `apps/pos/src/ui/errors.ts`, `apps/pos/src/ui/th.ts`, `apps/pos/test/helpers/db.ts` (D50 Q3-27: `PRICE_CHANGED`, `shownTotalSatang`)

**ราคาเปลี่ยนระหว่างตะกร้ากับตอนชำระ (D50 Q3-27 · I-7):** ตะกร้าเก็บราคาจาก `loadMenu` (cache) และ QR สร้างจากยอดตะกร้า แต่ `commitSale` คิดราคาใหม่จาก DB ณ `now` → ถ้ามีราคาใหม่มีผลระหว่างนั้น ลูกค้าจ่าย X แต่บิลบันทึก Y (ผิด §4.1 และลิ้นชักปิดยอดไม่ลง) · แก้: ผู้เรียกส่ง `expectedTotalSatang` = ยอดที่ลูกค้าเห็น · `commitSale` คิดราคาจาก DB แล้วถ้ายอดไม่ตรง → `PRICE_CHANGED: shown <X>, now <Y>` และไม่เขียนอะไรเลย · หน้าจอ (Task 12) โหลดเมนูใหม่ ปรับราคาในตะกร้า แสดงยอดเดิม → ยอดใหม่ แล้ว **ให้แคชเชียร์กดยืนยันอีกครั้ง** (ไม่ชำระให้เอง) · เทียบที่ยอดรวม (สิ่งที่ลูกค้าจ่ายจริง) — ถ้าราคาหลายรายการเปลี่ยนแต่ยอดรวมเท่าเดิม บิลบันทึกราคาต่อหน่วยใหม่และยอดยังตรงกับที่รับเงิน
...
- Produces:
  - `types.ts` เพิ่ม: `CommitSaleInput = { orderId: string; actorUserId: string; lines: { variantId: string; sweetnessId: string; qty: number }[]; discount: { amountSatang: number; reason: string } | null; payment: { method: 'CASH'; tenderedSatang: number } | { method: 'PROMPTPAY' }; expectedTotalSatang: number }` · `CommitSaleResult = { orderId; receiptNo: string; queueNo: number; businessDate: string; totalSatang: number; changeSatang: number | null; method: 'CASH' | 'PROMPTPAY' }` · `PosApi.commitSale(input): Promise<CommitSaleResult>`
  ...
  - `api/sale.ts`: `commitSale(db, deps, input)` — throw `EMPTY_CART` · `BAD_INPUT` · `NEEDS_SETUP` · `NO_OPEN_SHIFT` · `NO_PRICE` · `NO_RECIPE` · `DISCOUNT_TOO_BIG` · `PRICE_CHANGED` (D50 Q3-27) · `TENDER_TOO_LOW` · ส่ง `orderId` ซ้ำคืนผลเดิม (T17)
  - `test/helpers/db.ts` ใช้ต่อ: `openReadyApi` · เพิ่ม `shownTotalSatang(t: TestApi, lines: CommitSaleInput['lines'], discount: CommitSaleInput['discount']): Promise<number>` (ยอดที่ตะกร้าจะแสดงจากเมนูตอนนี้ — ใช้เป็น `expectedTotalSatang` ในเทสต์)
  - `api/errors.ts` เพิ่ม `'PRICE_CHANGED'` · `ui/th.ts` เพิ่ม `errPriceChanged` · `ui/errors.ts` เพิ่ม `PRICE_CHANGED: TH.errPriceChanged`
...
Run: `pnpm --filter @dayo/pos exec vitest run sale`
...
แก้ `apps/pos/src/api/errors.ts` — เพิ่มใน `PosErrorCode` ต่อจาก `'DISCOUNT_TOO_BIG'`:
```ts
  | 'PRICE_CHANGED' // D50 Q3-27 — detail "shown <X>, now <Y>"
```
แก้ `apps/pos/src/ui/th.ts` — เพิ่มต่อจาก `errDiscountTooBig`:
```ts
  errPriceChanged: 'ราคาเปลี่ยนแล้ว — กำลังปรับราคาในตะกร้า',
```
แก้ `apps/pos/src/ui/errors.ts` — เพิ่มใน `MESSAGES` ต่อจาก `DISCOUNT_TOO_BIG`:
```ts
  PRICE_CHANGED: TH.errPriceChanged,
```

เพิ่มท้าย `apps/pos/test/helpers/db.ts` (และเพิ่ม `CommitSaleInput` เข้า import type จาก `../../src/api/types`):
```ts
/**
...
ใช้ skill `committing-code` แล้ว:
```bash
git add apps/pos/src/db/stock.ts apps/pos/src/api/sale.ts apps/pos/test/sale.test.ts apps/pos/src/api/types.ts apps/pos/src/api/pos-api.ts apps/pos/src/api/errors.ts apps/pos/src/ui/errors.ts apps/pos/src/ui/th.ts apps/pos/test/helpers/db.ts
...
- Produces:
  ...
  - `test/helpers/db.ts` เพิ่ม: `sellSku(t: ReadyApi, sku: string, qty: number, payment: CommitSaleInput['payment'], discount?: CommitSaleInput['discount']): Promise<CommitSaleResult>` (ความหวาน S050, ผู้ขาย = `t.owner`, `expectedTotalSatang` จาก `shownTotalSatang` — D50 Q3-27)

- [ ] **Step 1: เพิ่ม helper และเขียนเทสต์ให้ตก**

เพิ่มท้าย `apps/pos/test/helpers/db.ts` (และที่หัวไฟล์เพิ่ม `import { eq } from 'drizzle-orm'`, `import * as s from '@dayo/db-schema/sqlite'` และเพิ่ม `CommitSaleResult` เข้า import type จาก `../../src/api/types` — `CommitSaleInput` มีแล้วจาก Task 10):
```ts
...
Run: `pnpm --filter @dayo/pos exec vitest run orders`
...
- Modify: `apps/pos/src/app/queries.ts`, `apps/pos/src/router.tsx`, `apps/pos/e2e/helpers.ts`, `apps/pos/src/ui/th.ts` (D50 Q3-27)

**Interfaces:**
- Consumes: `useCart`, `toSaleLines`, `cartTotals`, `cartReducer` + action `reprice` (Task 8/9) · `posErrorCode` (Task 4) · `menuKey` (Task 9) · `quickTenderOptions`, `cashChangeSatang` (Task 2) · `commitSale`, `promptPayForAmount`, `getOrder` (Task 10/11) · `QRCode.toDataURL` (qrcode 1.5.4)
- Produces:
  - `app/queries.ts` เพิ่ม: `ordersKey = ['orders']` · `orderKey(orderId) = ['order', orderId]`
  - `app/use-commit-sale.ts`: `type PriceChange = { fromSatang: number; toSatang: number }` · `useCommitSale(): { pay; priceChange: PriceChange | null }` — `pay` = mutation รับ `CommitSaleInput['payment']` ส่ง `expectedTotalSatang = cartTotals(state).totalSatang` · สำเร็จ → ไป `/done?orderId=…` → ล้างตะกร้า (orderId ใหม่) → invalidate bootstrap/menu/orders · `PRICE_CHANGED` → โหลดเมนูใหม่ (`fetchQuery` `menuKey`, `staleTime: 0`) → `reprice` ตะกร้า → `priceChange` = ยอดเดิม/ยอดใหม่ · **ไม่ชำระซ้ำให้เอง** แคชเชียร์ต้องกดยืนยันอีกครั้ง (D50 Q3-27)
  - `ui/th.ts` เพิ่ม: `priceChanged(from: string, to: string)` · `qrRescan`
  - route `/done` มี `validateSearch` → `{ orderId: string }` · `DONE_AUTO_CLOSE_MS = 5000` (spec §5)
  - testid: `cash-total`, `tender-exact`, `tender-<บาท>` (เช่น `tender-500`), `tender-input`, `cash-change`, `confirm-cash` · `qr-total`, `qr-image` (มี attribute `data-payload`), `qr-received`, `pay-back` · `price-changed` (ทั้งสองหน้าชำระ — D50 Q3-27) · `done-queue`, `done-receipt`, `done-change`, `done-line-<i>`, `done-new-sale`
...
แก้ `apps/pos/src/ui/th.ts` — เพิ่ม (D50 Q3-27):
```ts
...
ใช้ skill `committing-code` แล้ว:
```bash
git add apps/pos/src/app/use-commit-sale.ts apps/pos/src/screens/CashPayScreen.tsx apps/pos/src/screens/QrPayScreen.tsx apps/pos/src/screens/DoneScreen.tsx apps/pos/e2e/sell-cash.spec.ts apps/pos/src/app/queries.ts apps/pos/src/router.tsx apps/pos/e2e/helpers.ts apps/pos/src/ui/th.ts
...
- Produces:
  ...
  - `api/void.ts`: `voidOrder(db, deps, input)` — throw `BAD_INPUT` · `PIN_WRONG` · `PIN_LOCKED` (D50 Q3-21) · `NOT_OWNER` · `ORDER_NOT_FOUND` · `VOID_NOT_ALLOWED`
...
Run: `pnpm --filter @dayo/pos exec vitest run void`
...
pnpm --filter @dayo/pos e2e
```
Expected: PASS ทั้งหมด (first-run, sell-cash, sell-qr-void ×2) · `openOrder` ทนต่อ `done-new-sale` ปิดเองก่อนกดแล้ว (M20)
...
- [ ] **Step 4: ทดสอบบนแท็บเล็ตจริง (Android + Chrome — D46 · เปิดผ่าน Quick Tunnel — D48 Q3-2)** ⛔ HAND-OFF เจ้าของร้าน

> **ขั้นนี้เจ้าของทำเองบนแท็บเล็ต — agent ห้ามทำแทนหรือกรอกผลแทน.** Agent: commit Step 1–3, เตรียมหัวข้อในบันทึกการทำแผน (Step 5) ไว้ให้กรอก แล้ว **หยุด** แจ้งเจ้าของพร้อมคำสั่งใน Step 4 · ทำต่อ Step 5–8 เมื่อเจ้าของส่งผลที่วัดได้กลับมาเท่านั้น

**M16: ห้ามจดหมายเลขพร้อมเพย์จริง/PIN จริงลงไฟล์ใด ๆ ใน repo หรือภาพหน้าจอที่ commit (D48 Q3-3)** — ทั้งในบันทึกการทำแผน (Step 5), `ผลspike.md` และภาพหน้าจอทุกใบ
...
> **Keep one cloudflared process running from item 1 to item 5.** ถ้า URL เปลี่ยน แปลว่าเป็น origin ใหม่: เริ่มนับใหม่จากข้อ 1 และห้ามบันทึกว่าล้มเหลว สำหรับข้อ 7 (รีสตาร์ตแท็บเล็ต) ให้ใช้ `adb reverse tcp:4173 tcp:4173` + `http://localhost:4173` แทน — origin นั้นคงที่ และ localhost เป็น secure context อยู่แล้ว บันทึกในบันทึกการทำแผนว่าแต่ละข้อใช้วิธีไหน
...
10. เปิด DevTools remote (chrome://inspect) หรือดูป้าย `storage-not-persistent` — ต้องไม่ขึ้นหลังติดตั้งแอป
...
ทำเฉพาะเมื่อ Step 7 ผ่านและไม่มี issue ค้าง (D42):
```bash
git status --short            # ต้องว่าง
git checkout main
git merge --no-ff plan-3-pos-sell
...
## Self-review (ทำใหม่ 18 ก.ย. 2026 หลังรับคำตอบ D44–D49 และ Plan 2 ฉบับ final review · ปรับตาม D50 Q3-20–27 วันเดียวกัน)
...
| §7 PIN: กันเดา — ผิด 5 ครั้งติดรอ 30 วิ เพิ่มเท่าตัวถึง 15 นาที · audit (D50 Q3-21) | Task 6 (`pin-guard.ts`, ใช้ร่วม login + `requireOwnerPin`) · Task 13 |
| §4.1 Σ payment = total: ยอดที่ลูกค้าเห็น = ยอดที่บันทึก (I-7, D50 Q3-27) · ไม่มีบิล 0 บาท (D50 Q3-20) | Task 8 (`reprice`, ส่วนลด < ยอด) · Task 10 (`expectedTotalSatang` → `PRICE_CHANGED`) · Task 12 (ปรับราคา + ยืนยันใหม่) |
| §12 "ยังไม่ส่ง N รายการ" บนจอตลอด | Task 4 (`pendingSyncItems` = `countPendingSyncItems` นับบิล ไม่นับแถว outbox — D50 Q3-26) · Task 9 (`pending-sync`) |
| §8 E2E ขาย → ชำระ → ตัดสต็อก → ยกเลิก · ออฟไลน์ขายต่อ | Task 12, 14, 15 (สต็อกตรวจละเอียดใน integration Task 10, 13) |

**ไม่อยู่ในแผนนี้ (ตั้งใจ):** ปิดกะ/นับธนบัตร/X/Z และปุ่มสำรองไฟล์ → **แผน 3b** (D48 Q3-17 · อยู่ใน README แผนแล้ว) · deploy Cloudflare Pages → งานเล็กแยกหลังแผน 3 (D48 Q3-2) · ส่ง outbox/sync + ส่วน "ปิดกะ → sync ถึงเซิร์ฟเวอร์" ของ e2e §8 → แผน 5, 8 · `order_payment_intent`, สถานะ `ready`/`picked_up`, ออเดอร์ LINE (D47 ข้อ 1) → แผน 6 · ต้นทุนมาตรฐาน/ของไม่นับสต็อก (D47 ข้อ 3–4) → แผน 4/5 · VAT เปิดใช้ → config ปิดไว้ (`VAT_OFF`) · "เปิดกะด่วน" ตามสเปก §4.8 (เฉพาะเจ้าของเปิดได้ + เงินทอน 0 + มี event บันทึก) **ยังไม่ทำ** — แผนนี้แค่ให้ช่องเงินทอนรับ 0 ได้ ไม่มีการบังคับสิทธิ์เจ้าของหรือ event แยก ต่างจากสเปก → เลื่อนไปทำเต็มรูปแบบใน **แผน 3b** (M18) · staff role เตรียมไว้ใน `requireOwnerPin` แต่ยังไม่มีหน้าจัดการผู้ใช้

**2. การตัดสินใจ D1–D50 ที่แผนนี้แตะ:** D2/D4 ไม่ผูกอุปกรณ์ ไม่พิมพ์ · D6 เขียนลงเครื่องก่อน + UUIDv7 · D7 เงินสด + QR ใส่ยอด · D8 VAT ปิด + เลขบิลต่อเนื่อง + ห้ามลบบิล · D12 ความถูกต้อง (transaction เดียว, เทสต์ rollback/concurrent, pin id seed) · D13 stack · D16/D24 seed จากไฟล์ครั้งเดียว · D18 ตัดสต็อกตอนจ่าย + ยกเลิกถามทำแล้วหรือยัง · D21 PIN แยกคน + audit_log ตอนตั้งค่า + เหตุผลส่วนลด/ยกเลิก · D22 กะ = วันทำการ · D27 · D28 · D29 · D33 usat · D34 standard cost ก่อนมี movement · D36 · D38 · D39 · D42 audit + review ก่อน merge · D43 โมเดล · D44 แบรนด์ (token สี · ไอคอน · โลโก้ตัดสโลแกน · ตาราง contrast) · D45 commit (skill · ไม่มี trailer · stage ตามชื่อ) · D46 Android + Chrome · D47 ข้อ 2, 5–9 · D48 ทุกข้อ · D49 ไม่ push · ข้อที่ไม่เกี่ยว (D10, D14–D15, D17, D19–D20, D25–D26, D30–D32, D35, D37, D40–D41, D47 ข้อ 1/3/4) อยู่ในแผนอื่น · **D50** (Q3-20–27): ไม่มีบิล 0 บาท · ล็อก PIN 5 ครั้ง/30 วิ ×2 สูงสุด 15 นาที · เจ้าของอนุมัติ void ตัวเองได้ด้วย PIN ซ้ำ + เหตุผล · คลื่นส้ม + เขียวเข้ม ไม่มี Matcha · ล็อกแล้วเก็บตะกร้า · ขายดี 7 วันปฏิทิน · ป้ายนับบิล · `PRICE_CHANGED` + ยืนยันใหม่

**3. Placeholder scan:** ค้น `TBD|TODO|implement later|similar to|รอคำตอบ|รอ Q3` แล้วไม่พบ (D50 ตอบ Q3-20–27 ครบ 18 ก.ย.) · ช่องที่ต้องกรอกเฉพาะไฟล์บันทึกผล (ผล spike, บันทึกการทำแผน) ซึ่งเป็นแบบฟอร์มให้กรอกค่าจริง · ค่าที่วัดจากไฟล์แบรนด์ (`viewBox` 6032, contrast) คำนวณจากไฟล์จริงตอนเขียนแผน และมีขั้น ⛔ HAND-OFF ให้เจ้าของตรวจด้วยตาจากภาพที่ agent ถ่ายไว้ (I-10)

**4. Type consistency:** `PosApi` มีเมธอด 10 ตัว (bootstrap, setupShop, login, openShift, loadMenu, commitSale, listOrders, getOrder, promptPayForAmount, voidOrder) และ `POS_API_METHODS` ถูกตรวจด้วย type `POS_API_METHODS_COMPLETE` · ชื่อที่ใช้ข้าม task: `RemoteDb`, `BundledMigration`, `migrateSqliteRemote`, `SQLITE_MIGRATIONS`/`SQLITE_TABLES`/`SQLITE_TRIGGERS`, `oo1Callback`, `nodeSqliteCallback`, `canonicalSeedOpts`, `SEED_ID_NAMESPACE`, `SEED_EFFECTIVE_FROM`, `seedToRows`, `deterministicSeedId`, `applySeedSqlite`, `loadCatalogSqlite`, `isSeeded`, `initDatabase`, `createPosApi`, `ApiDeps`, `PosError`/`posErrorCode`, `enqueueOutbox`, `appendOrderEvents`, `loadDeviceChain`, `requireDevice`, `currentOpenShift`, `requireStoreChannelId`, `getSetting(db, key, atIso)`, `loadSaleContext`, `insertMovements`, `promptPayForAmount(db, deps, amount)`, `getOrder`, `requireOwnerPin(db, deps, userId, pin)`, `pinLockMs`/`PinGuardState` (D50 Q3-21), `countPendingSyncItems`/`BootstrapState.pendingSyncItems` (D50 Q3-26), `CommitSaleInput.expectedTotalSatang`/`PRICE_CHANGED`/`shownTotalSatang`/`useCommitSale(): { pay; priceChange }`/cart action `reprice` (D50 Q3-27), `openReadyApi`, `sellSku`, `TEST_SETUP`, `PINS`, `TEST_PIN_COST`, `BrandBar` — ตรวจตรงกันทุกจุด · ค่า enum ใช้ชนิดจาก `textEnum` ของแผน 2 (`UserRole`, `OrderStatus`, `PaymentMethod`, `CashMovementKind`, `OutboxStatus`, `EventType`, `ActorType`) ไม่มี cast · แถวที่สร้างก่อน insert ใช้ `satisfies typeof s.<table>.$inferInsert`

**5. ความเสี่ยงที่รู้อยู่:** `sqlite-proxy` ส่ง `begin/commit` ผ่าน callback — ปลอดภัยเพราะทุกเมธอดวิ่งผ่าน `createSerialQueue` (เทสต์ concurrent ใน Task 10) · `migrateSqliteRemote` ใช้ `dialect`/`session` ที่ drizzle ประกาศเป็น internal (ตัวเดียวกับที่ migrator ของ drizzle เองเรียก) → ถ้าอัปเกรด drizzle ต้องรันเทสต์ `browser-migrate` ซ้ำ · `opfs-sahpool` เปิดได้ทีละแท็บ → `DbErrorScreen` บอกให้ปิดแท็บอื่น · `node:sqlite` ยังเป็น experimental ใน Node 22 (เตือนได้ ไม่ใช่ error) · CHECK ของ D47 ข้อ 7 เพิ่มโดยอีก branch ขณะเขียนแผน — Task 1 Step 1 บังคับอ่าน CHECK จริงและปรับแถวที่แผนนี้เขียนให้ผ่าน · รุ่นแท็บเล็ตยังไม่กำหนด (D46) → spike ต้องรันซ้ำเมื่อได้เครื่องจริง · หลัง merge แผน 2 รอบสุดท้าย (CHECK ของ D47 ข้อ 7 มาแล้ว) พบว่า `payment.amount_satang > 0` ทำให้บิล 0 บาทบันทึกไม่ได้ → เพิ่ม Q3-20 และใช้ค่าแนะนำ (ส่วนลด < ยอดรวม) ใน Task 8–10
