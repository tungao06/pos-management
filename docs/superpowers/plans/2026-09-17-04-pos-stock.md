...
- **ต้นแบบที่ทดสอบแล้ว:** โค้ดทุกไฟล์ในแผนนี้ถูกเขียนและรันจริงก่อนเขียนแผน (ทั้ง repo เขียว · e2e 13/13) สำเนาอยู่ที่ `.superpowers/plan-4-prototype/` (ไม่เข้า git — D45) ใช้เทียบได้ถ้าสงสัย แต่ **แผนนี้คือแหล่งความจริง** — ทำตามขั้นตอนทีละ task ห้ามคัดลอกทั้งโฟลเดอร์ทีเดียว
...
│                           qtyPerUnitMilli · assertUnitsMilli · badInputOnRange
├── stock-overview.ts  ใหม่: stockOverview · openCountId · COUNT_DUE_DAYS
...
├── errors.ts          แก้: + PRICE_JUMP · STOCK_COUNT_NOT_OPEN
...
├── CountScreen.tsx     ใหม่ (/stock/count) + CountScreen.test.tsx
...
จำนวนเทสต์ตั้งต้น (main 1a608b2): contracts 5 · domain 148 · excel-import 33 · db-schema 121 · pos 173 · e2e 9
...
git merge-base --is-ancestor main plan-4-draft && echo OK   # draft ต้องต่อจาก main ล่าสุด (ถ้าไม่ OK: rebase draft ก่อน แล้วถามเจ้าของ)
git checkout -b plan-4-stock plan-4-draft
pnpm install --frozen-lockfile
grep -n "Q4-" docs/design/00-บันทึกการตัดสินใจ.md   # ต้องเจอแถว D56 (คำตอบ Q4-1 … Q4-12) — ไม่เจอ = หยุด ถามเจ้าของ
pnpm exec turbo run typecheck test --force          # ตั้งต้นต้องเขียวทั้ง repo (11/11 tasks)
```
Expected: branch `plan-4-stock` ต่อจาก `plan-4-draft` · เจอ D56 · ทุกแพ็กเกจผ่าน (contracts 5 · domain 148 · excel-import 33 · db-schema 121 · pos 173) · ถ้าคำตอบ D56 ข้อใดต่างจากค่าแนะนำ → แก้ตามตาราง "คำตอบของเจ้าของที่แผนนี้ใช้" ก่อน
...
git commit -m "feat(domain): purchase cost, stock levels, stock-out and opening count rules"
```
...
**Files:**
- Modify: `packages/contracts/src/enums.ts`, `packages/db-schema/src/sqlite/stock.ts`, `packages/db-schema/src/pg/stock.ts`, `packages/db-schema/test/migrate.test.ts`, `packages/db-schema/test/parity.test.ts`
...
Run: `pnpm --filter @dayo/db-schema exec vitest run migrate parity`
Expected: FAIL — ได้ 38 ตาราง ไม่ใช่ 39 · `pg table stock_adjustment missing`
...
Run: `pnpm exec turbo run typecheck test --force`
Expected: 11/11 ผ่าน · db-schema 121 (จำนวนเท่าเดิม — แก้ค่าคาดหวัง) รวม `drift.test.ts` (schema ตรงกับ migration ที่ commit) และ `browser-migrate.test.ts` (ตารางในเครื่องตรงกับ `SQLITE_TABLES`) · pos 173 ผ่าน (แอปรัน migration ใหม่ตอนเปิด — `init.test.ts`)
...
git commit -m "feat(db-schema): stock_adjustment header for stock-outs with a reason"
```
...
- Produces:
  - `stock-common.ts`: `KEY_COUNT_ITEM_CODES: readonly string[]` (Q4-2) · `MAX_UNITS_MILLI = 9_999_999` · `MAX_LINE_SATANG = 10_000_000` · `MAX_STOCK_LINES = 50` · `stockBusinessDate(db, deviceId, atIso): Promise<string>` (Q4-1) · `type StockItemKind = 'raw' | 'prepared'` · `requireStockItem(db, itemId, kinds): Promise<item row>` (BAD_INPUT ถ้าไม่ active/ไม่ tracked/ผิดชนิด) · `qtyPerUnitMilli(db, itemId, purchaseUnitId | null): Promise<number>` · `assertUnitsMilli(unitsMilli, name): void` · `badInputOnRange<T>(fn: () => T): T`
  - `stock-overview.ts`: `COUNT_DUE_DAYS = 7` (Q4-12) · `openCountId(db, deviceId): Promise<string | null>` (Task 7 ใช้) · `stockOverview(db, deps): Promise<StockOverviewDto>`
  - `shift.ts`: `export async function requireActiveUser(db, userId)` (เดิม private — Task 4–7 ใช้)
  - `types.ts`: `PurchaseUnitDto` · `BomLineDto` · `BaseBatchDto` · `StockItemDto` · `StockOverviewDto` · `PosApi.stockOverview(): Promise<StockOverviewDto>`
  - `errors.ts`: `PRICE_JUMP` (Task 4) · `STOCK_COUNT_NOT_OPEN` (Task 7) — ข้อความไทย `TH.errPriceJump`, `TH.errStockCountNotOpen`
  - `outbox.ts`: `OutboxTable` + `'purchase' | 'purchase_line' | 'production_batch' | 'stock_count' | 'stock_count_line' | 'stock_adjustment'`
  - `bootstrap.ts`: `countPendingSyncItems` นับ movement ใต้ `refType:refId` และ `purchase_line`/`stock_count_line` ใต้หัวใบ (T4-8)
  - `test/helpers/stock.ts`: `itemId(t, code)` · `defaultUnitId(t, code)` · `stockOf(t, code)` · `movementsOf(t, refType, refId)` · `outboxKeys(t)`
...
describe('stockOverview (spec §5 หน้าสต็อก)', () => {
  it('lists tracked raw items and bases only, at the standard cost before any movement; nothing on hand = out', async () => {
    ...
    expect(tea).toMatchObject({ kind: 'raw', useUnit: 'g', onHandMilli: 0, avgCostUsat: 19_250_000, valueSatang: 0, reorderPointMilli: 400_000, status: 'out', alert: true, isKeyCount: true, latestBatch: null, bom: null })
    ...
    // Q4-11: an empty base is "out" but not an alert (bases have no reorder point)
    ...
    expect(o).toMatchObject({ businessDate: '2026-09-17', totalValueSatang: 0, alertCount: 31, expiredBaseCodes: [], lastCountAt: null, countDue: true, openCountId: null })
  })
...
```ts
  | 'PRICE_JUMP' // D47 item 3: a received price is > 10% off the current cost — detail = the item codes; retry with acceptPriceJump
...
```ts
  // แผน 4
  errPriceJump: 'ราคาบางรายการต่างจากต้นทุนเดิมเกิน 10% — ตรวจราคาอีกครั้ง แล้วกด "ยืนยันราคานี้"',
 ...
 * `refType:refId`). Any other record (a shift, a paid-in/out) counts once per row id. Only status pending — plan 2
 * M8; dead rows are shown elsewhere by plan 5.
 */
...
/** A quantity typed in a unit is at most 9,999.999 units — what `parseQtyInput` accepts (typo guard, like MAX_CASH_MOVEMENT_SATANG). */
export const MAX_UNITS_MILLI = 9_999_999
...
import { KEY_COUNT_ITEM_CODES, stockBusinessDate } from './stock-common'
...
export type StockItemDto = {
  ...
  status: StockStatus
  /** raw: low / out / negative · base: negative or expired (Q4-10, Q4-11) — what the sell-screen badge counts. */
  alert: boolean
...
Run: `pnpm exec turbo run typecheck test --force`
Expected: 11/11 ผ่าน · pos 173 → **177** (stock-overview 3 · pending-sync +1) · เคสเดิมใน `pending-sync.test.ts` (movement `refType: 'test'`) ยังนับ 1 เหมือนเดิม
...
git commit -m "feat(pos): stock overview api and stock documents in the sync badge"
```
...
- Test: `apps/pos/test/purchase.test.ts` (ใหม่)

**Interfaces:**
- Consumes: `isPriceJump`, `purchaseMovements`, `purchaseUnitCostUsat`, `unitsToUseMilli`, `PurchaseLineDraft` (Task 1) · `requireStockItem`, `qtyPerUnitMilli`, `assertUnitsMilli`, `badInputOnRange`, `stockBusinessDate`, `MAX_LINE_SATANG`, `MAX_STOCK_LINES` (Task 3) · `requireActiveUser` (Task 3) · `insertMovements`, `loadCostStates`, `makeCostOf` (`db/stock.ts`) · `enqueueOutbox` · `currentOpenShift`, `requireDevice` · `loadCatalogSqlite` · `REASON_MAX_LENGTH`
- Produces:
  ...
  - `purchase.ts`: `receivePurchase(db, deps, input)` — throw `BAD_INPUT` (ไม่มีบรรทัด/เกิน 50, จำนวน/ยอดผิด, ของ untracked/เบส/ไม่ active, หน่วยไม่ใช่ของสินค้านั้น, ร้าน/หมายเหตุเกิน 200, ผู้ใช้ไม่มี) · `PRICE_JUMP` (detail = รหัสสินค้าคั่นด้วย `,`) · `NO_OPEN_SHIFT` (จ่ายจากลิ้นชักตอนไม่มีกะ)
...
describe('receivePurchase (spec §5 รับของเข้า · D19 · §4.4)', () => {
  ...
  it('a price more than 10% off the current cost is refused with PRICE_JUMP (nothing written) until confirmed (D47 item 3)', async () => {
    const t = await openReadyApi()
    ...
    // exactly +10% is not a jump
    await t.api.receivePurchase(input(t, [await teaBags(t, 1_000, 9_350)])) // 0.23375 vs avg 0.2125 = +10.0%
  })
...
- [ ] **Step 2: เขียน `apps/pos/src/api/purchase.ts`**
...
import { assertUnitsMilli, badInputOnRange, MAX_LINE_SATANG, MAX_STOCK_LINES, qtyPerUnitMilli, requireStockItem, stockBusinessDate } from './stock-common'
 ...
 * item_cost_state + outbox, in one transaction. Tracked raw items only (D29). A unit cost more than 10% off the current
 * cost is refused with PRICE_JUMP until the person confirms it (D47 item 3). With `paidFromDrawer` the total also leaves
 * the drawer as a PAID_OUT of the open shift, in the same transaction (Q4-6).
 */
export async function receivePurchase(db: RemoteDb, deps: ApiDeps, input: ReceivePurchaseInput): Promise<PurchaseDto> {
  ...
  if (supplier.length > REASON_MAX_LENGTH || note.length > REASON_MAX_LENGTH) throw new PosError('BAD_INPUT', `supplier and note are at most ${REASON_MAX_LENGTH} characters`)
  ...
  return db.transaction(async (tx) => {
    ...
    for (const l of input.lines) {
      ...
      if (isPriceJump(unitCostUsat, costOf(item.id))) jumps.push(item.code)
    ...
    if (input.paidFromDrawer) {
      ...
      if (totalSatang <= 0) throw new PosError('BAD_INPUT', 'nothing to pay from the drawer')
      const cashRow = {
        id: deps.newId(),
        shiftId: shift.id,
        kind: 'PAID_OUT',
        ...
        reason: (supplier === '' ? 'รับของ' : `รับของ ${supplier}`).slice(0, REASON_MAX_LENGTH),
        createdBy: actor.id,
        createdAt: at,
      } satisfies typeof s.cashMovement.$inferInsert
      await tx.insert(s.cashMovement).values(cashRow)
      await enqueueOutbox(tx, 'cash_movement', cashRow, at, deps.newId)
...
Run: `pnpm exec turbo run typecheck test --force`
Expected: 11/11 ผ่าน · pos 177 → **181** · ถ้า insert `cash_movement` ตกด้วย CHECK `cash_movement_void_refund_order_ck` แปลว่าส่ง `orderId` ไม่เป็น `null` — แก้โค้ด ห้ามแก้ schema

- [ ] **Step 5: Commit**

ใช้ skill `committing-code` แล้ว:
```bash
git add apps/pos/src/api/purchase.ts apps/pos/src/api/types.ts apps/pos/src/api/pos-api.ts apps/pos/test/purchase.test.ts
git commit -m "feat(pos): receive goods at a moving average with a price-jump check"
```
...
- Produces:
  ...
  - `production.ts`: `MIN_SCALE_BP = 1_000` · `MAX_SCALE_BP = 50_000` · `MAX_YIELD_FACTOR = 2` (T4-12) · `produceBatch(db, deps, input)` — throw `BAD_INPUT` (ไม่ใช่เบส tracked, ไม่มี BOM ปัจจุบัน, scale นอกช่วง, yield ≤ 0 หรือเกิน 2 เท่าของมาตรฐาน)
...
describe('produceBatch (spec §4.4 ทำเบส · §4.6 · D17)', () => {
  ...
  it('refuses a raw item, a scale outside 0.1×–5×, a zero yield or more than twice the standard yield — writing nothing', async () => {
    const t = await openReadyApi()
    const base = await itemId(t, 'PB-TEA-THAI')
...
/** T4-12: the actual yield may be at most twice the standard yield of the chosen scale (typo guard). */
...
export async function produceBatch(db: RemoteDb, deps: ApiDeps, input: ProduceBatchInput): Promise<ProductionBatchDto> {
  ...
  return db.transaction(async (tx) => {
    ...
    const maxYield = scaleQtyMilli(bom.yieldMilli, input.scaleBp) * MAX_YIELD_FACTOR
    if (input.yieldActualMilli > maxYield) throw new PosError('BAD_INPUT', `actual yield ${input.yieldActualMilli} is more than ${MAX_YIELD_FACTOR}× the standard yield`)
    const costOf = makeCostOf(catalog, await loadCostStates(tx))
...
Run: `pnpm exec turbo run typecheck test --force`
Expected: 11/11 ผ่าน · pos 181 → **185** · ต้นทุนชาไทยเบส 1 batch ก่อนมีของเข้า = 2,000,000 usat/ml (ตรง spec §9 "ชาไทยเบส 2,000,000 usat/ml") · ถ้าได้ 1,992,500 แปลว่าอ่าน standard cost ของ `RM-TEA-02` ผิด (ของจริงในไฟล์ร้าน 19,125,000 — ตัวเลข 19,000,000 เป็นแค่ตัวอย่างในเทสต์ domain เดิม)
...
git commit -m "feat(pos): record a base batch with its cost and expiry"
```
...
- Produces:
  ...
  - `adjust.ts`: `MAX_ADJUST_DRINKS = 99` · `DISCARD_EXPIRED_REASON = 'ทิ้งเบสหมดอายุ'` · `DISCARD_LEFTOVER_REASON = 'ทิ้งเบสที่เหลือ'` · `adjustStock` — throw `BAD_INPUT` (ประเภทผิด, ไม่มีเหตุผล/เกิน 200, ไม่มีรายการ, ของ untracked, จำนวนผิด) · `NO_RECIPE` · `discardBase` — throw `BAD_INPUT` (ไม่ใช่เบส / ไม่มีของเหลือ)
...
import { PosError } from './errors'
import { requireActiveUser } from './shift'
import { assertUnitsMilli, badInputOnRange, MAX_STOCK_LINES, qtyPerUnitMilli, requireStockItem, stockBusinessDate } from './stock-common'
import { REASON_MAX_LENGTH, type AdjustStockInput, type DiscardBaseInput, type StockAdjustmentDto } from './types'
...
export async function adjustStock(db: RemoteDb, deps: ApiDeps, input: AdjustStockInput): Promise<StockAdjustmentDto> {
  ...
  if (reason === '') throw new PosError('BAD_INPUT', 'a stock adjustment needs a reason')
...
Run: `pnpm exec turbo run typecheck test --force`
Expected: 11/11 ผ่าน · pos 185 → **190**
...
git commit -m "feat(pos): stock-outs with a reason by item or by recipe, and discarding a base"
```

---

### Task 7: นับสต็อก — ใบนับเปิด/บันทึกรายการ/ปิด (ยอดยกมา D30 · COUNT_ADJ · expected แช่แข็งรายบรรทัด) 💰 review: opus
...
- Produces:
  ...
  - `stock-count.ts`: เมธอดข้างบน + `loadStockCount(db, countId)` · throw `STOCK_COUNT_NOT_OPEN` (ใบนับปิดแล้ว/ไม่มี/ของเครื่องอื่น) · `BAD_INPUT` (ของ untracked, จำนวนติดลบ/ทศนิยมเกิน/เกิน 9,999.999 หน่วย, หน่วยผิดสินค้า, ผู้ใช้ไม่มี) · นับ 0 ได้
  - ผลใน DB: เปิด = `stock_count` (open) + outbox `stock_count:<id>` · บันทึกรายการ = upsert `stock_count_line` (ยังไม่เข้า outbox — T4-11) · ปิด = แก้ variance ของทุกบรรทัดด้วยต้นทุนตอนปิด + outbox ทุกบรรทัด + movement `OPENING` (ของที่ไม่เคยอยู่ในใบนับที่ปิดแล้ว — D30, ราคา standard) / `COUNT_ADJ` (ราคาเฉลี่ย) + `stock_count` closed + outbox `stock_count:<id>:closed`
...
describe('stock count (spec §4.5 · §5 นับสต็อก)', () => {
  it('first count = opening balance at the standard cost (D30); expected is frozen per line when counted, so a later sale is not counted twice (T4-2)', async () => {
    const t = await openReadyApi()
    const tea = await itemId(t, 'RM-TEA-01')
    const milk = await itemId(t, 'RM-MLK-02')
    await sellSku(t, 'Original-16oz', 1, { method: 'CASH', tenderedSatang: 4_500 }) // evaporated milk −46 ml
    ...
    const after = await t.api.saveCountLine({ actorUserId: t.owner.id, countId: c.id, itemId: milk, purchaseUnitId: await defaultUnitId(t, 'RM-MLK-02'), countedUnitsMilli: 1_000 })
    expect(after.lines).toEqual([
      { itemId: milk, code: 'RM-MLK-02', name: expect.any(String), useUnit: 'ml', purchaseUnitId: expect.any(String), unitName: 'กระป๋อง', countedUnitsMilli: 1_000, countedUseMilli: 405_000, expectedUseMilli: -46_000, varianceUseMilli: 451_000, varianceSatang: 3_341, opening: true },
    ...
    expect(await t.db.select().from(s.stockMovement).where(eq(s.stockMovement.refType, 'stock_count')).all()).toEqual([]) // nothing moves until the count closes
    expect((await outboxKeys(t)).filter((k) => k.startsWith('stock_count_line:'))).toEqual([]) // draft lines are not synced (T4-11)

    await sellSku(t, 'Original-16oz', 1, { method: 'CASH', tenderedSatang: 4_500 }) // sold after the milk was counted
    t.clock.advanceMs(60_000)
    const closed = await t.api.closeStockCount({ actorUserId: t.other.id, countId: c.id })
    expect(closed).toMatchObject({ status: 'closed', closedAt: '2026-09-17T03:01:00.000Z', totalVarianceSatang: 18_741 })
    expect(await movementsOf(t, 'stock_count', c.id)).toEqual([
      ...
      { code: 'RM-MLK-02', kind: 'OPENING', qtyMilli: 451_000, unitCostUsat: 7_407_407, businessDate: '2026-09-17' },
    ])
    // 405 ml counted, minus the 46 ml sold after counting — not 405 ml, and not 405 − 92
    expect(await stockOf(t, 'RM-MLK-02')).toEqual({ onHandMilli: 359_000, avgCostUsat: 7_407_407 })
    ...
    expect(keys.filter((k) => k.startsWith('stock_count_line:'))).toHaveLength(2)
    expect(await t.api.getOpenStockCount()).toBeNull()
    const o = await t.api.stockOverview()
    expect(o).toMatchObject({ lastCountAt: '2026-09-17T03:01:00.000Z', countDue: false, openCountId: null })
    expect(o.items.find((i) => i.code === 'RM-TEA-01')).toMatchObject({ onHandMilli: 800_000, status: 'ok', valueSatang: 15_400 })
  })

  it('a later count is COUNT_ADJ at the moving average; a line can be re-counted or taken out before closing', async () => {
    const t = await openReadyApi()
    const tea = await itemId(t, 'RM-TEA-01')
    const bag = await defaultUnitId(t, 'RM-TEA-01')
    const first = await t.api.startStockCount(t.owner.id)
    await t.api.saveCountLine({ actorUserId: t.owner.id, countId: first.id, itemId: tea, purchaseUnitId: bag, countedUnitsMilli: 2_000 })
    await t.api.closeStockCount({ actorUserId: t.owner.id, countId: first.id })

    const c = await t.api.startStockCount(t.owner.id)
    expect(c.id).not.toBe(first.id)
    await t.api.saveCountLine({ actorUserId: t.owner.id, countId: c.id, itemId: tea, purchaseUnitId: bag, countedUnitsMilli: 1_000 })
    const recount = await t.api.saveCountLine({ actorUserId: t.owner.id, countId: c.id, itemId: tea, purchaseUnitId: bag, countedUnitsMilli: 1_500 }) // counted again: replaces the line
    ...
    const withMilk = await t.api.saveCountLine({ actorUserId: t.owner.id, countId: c.id, itemId: await itemId(t, 'RM-MLK-01'), purchaseUnitId: null, countedUnitsMilli: 0 }) // 0 is a valid count
  ...
  it('closing a count with no lines just ends it — no movement, and it does not count as "counted" (Q4-12)', async () => {
    ...
    expect(await t.db.select().from(s.stockMovement).all()).toEqual([])
    expect(await t.api.stockOverview()).toMatchObject({ lastCountAt: null, countDue: true })
  })
  ...
  it('refuses a closed count, untracked items, a negative or fractional count, a foreign unit — changing nothing', async () => {
    ...
    for (const bad of [
      ...
      { ...line, countedUnitsMilli: 10_000_000 },
...
import { openCountId } from './stock-overview'
...
/** Items that have been in a closed count before — everything else is counted for the first time (D30 opening). */
async function countedBefore(db: RemoteDb, itemIds: readonly string[]): Promise<Set<string>> {
  ...
  const rows = await db
    ...
    .where(and(eq(s.stockCount.status, 'closed'), inArray(s.stockCountLine.itemId, [...itemIds])))
    .all()
...
/** Cost of a counted line: the standard cost for a first count (D30), otherwise the moving average (spec §4.5). */
function lineCost(catalog: Catalog, costOf: (id: string) => number, itemId: string, opening: boolean): number {
  return opening ? requireItem(catalog, itemId).standardCostUsat : costOf(itemId)
}
...
export async function loadStockCount(db: RemoteDb, countId: string): Promise<StockCountDto> {
  ...
  const rows = await db
    ...
    .all()
  const before = count.status === 'open' ? await countedBefore(db, rows.map((r) => r.line.itemId)) : new Set<string>()
  const openingIds = count.status === 'closed' ? await openingItemIdsOf(db, countId) : null
  const lines: StockCountLineDto[] = rows.map((r) => ({
    ...
    opening: openingIds === null ? !before.has(r.line.itemId) : openingIds.has(r.line.itemId),
  }))
  return {
    id: count.id,
...
/** Items whose movement from this (closed) count was their OPENING (a first count with no variance writes none). */
async function openingItemIdsOf(db: RemoteDb, countId: string): Promise<Set<string>> {
  const rows = await db
    .select({ itemId: s.stockMovement.itemId })
    .from(s.stockMovement)
    .where(and(eq(s.stockMovement.refType, 'stock_count'), eq(s.stockMovement.refId, countId), eq(s.stockMovement.kind, 'OPENING')))
    .all()
  return new Set(rows.map((r) => r.itemId))
}
...
/**
 * Saves (or re-saves) one counted line of the open count. expected = the book on-hand at this moment — frozen per line
 * when the item is counted (spec §3.3 "ณ เวลานับ", T4-2), so sales between counting it and closing the count are not
 * double-counted. Draft lines are not synced until the count closes (T4-11).
 */
export async function saveCountLine(db: RemoteDb, deps: ApiDeps, input: SaveCountLineInput): Promise<StockCountDto> {
  ...
  await db.transaction(async (tx) => {
    ...
    const opening = !(await countedBefore(tx, [item.id])).has(item.id)
    const r = badInputOnRange(() =>
      countLineResult({
        itemId: item.id,
        expectedUseMilli: states.get(item.id)?.onHandMilli ?? 0,
        countedUnitsMilli: input.countedUnitsMilli,
        qtyPerUnitMilli: perUnit,
        avgCostUsat: lineCost(catalog, makeCostOf(catalog, states), item.id, opening),
      }),
    )
    const values = {
 ...
 * or OPENING at the standard cost for an item's first count ever (D30) — then the count is closed, and every line, the
 * closed count and the movements are queued for sync, in one transaction. A count closed with no lines just ends it.
 */
export async function closeStockCount(db: RemoteDb, deps: ApiDeps, input: CloseStockCountInput): Promise<StockCountDto> {
  ...
  await db.transaction(async (tx) => {
    ...
    const catalog = await loadCatalogSqlite(tx)
    const costOf = makeCostOf(catalog, await loadCostStates(tx))
    const before = await countedBefore(tx, lines.map((l) => l.itemId))
    ...
    for (const l of lines) {
      ...
      const r = badInputOnRange(() =>
        countLineResult({ itemId: l.itemId, expectedUseMilli: l.expectedUseMilli, countedUnitsMilli: l.countedUnitsMilli, qtyPerUnitMilli: perUnit, avgCostUsat: lineCost(catalog, costOf, l.itemId, opening.has(l.itemId)) }),
      )
      results.push(r)
...
export type StockCountLineDto = {
  ...
  /** First count of this item ever: closing writes its opening balance, OPENING at the standard cost (D30). */
  opening: boolean
}
...
Run: `pnpm exec turbo run typecheck test --force`
Expected: 11/11 ผ่าน · pos 190 → **194** · เคสแรกคือหลักฐานของ T4-2: นม 405 ml ที่นับได้ ลบของที่ขายหลังนับ 46 ml = 359 ml (ถ้าได้ 405 แปลว่าคิด expected ตอนปิด · ถ้าได้ 313 แปลว่าคิด expected ตอนเปิดใบนับ)

- [ ] **Step 5: Commit**

ใช้ skill `committing-code` แล้ว:
```bash
git add apps/pos/src/api/stock-count.ts apps/pos/src/api/types.ts apps/pos/src/api/pos-api.ts apps/pos/test/stock-count.test.ts
git commit -m "feat(pos): stock count with opening balances and per-line frozen book figures"
```
...
- Produces:
  ...
  - หน้าขาย: `nav-stock` (ข้อความ `TH.stockMenuAlerts(n)` เมื่อมีคำเตือน — Q4-10) · `base-expired` (แถบเบสหมดอายุ ไม่บล็อกการขาย)
...
const STOCK: StockOverviewDto = {
  ...
  items: [
    {
      itemId: 'i-shot', code: 'PB-MATCHA-SHOT', name: 'มัทฉะช็อต', kind: 'prepared', category: 'เบส', useUnit: 'ml', onHandMilli: 120_000, avgCostUsat: 95_666_667,
...
```ts
  ...
  receiveJump: (pct: string): string => `ราคาต่างจากต้นทุนเดิม ${pct}`,
  ...
  countBlindHint: 'นับของจริงแล้วกรอก — ระบบจะแสดงยอดตามบัญชีและส่วนต่างหลังบันทึกแต่ละรายการ',
  countSaveLine: 'บันทึก',
  countRecount: 'นับใหม่',
...
```
Expected: 11/11 ผ่าน · pos 194 → **199** (qty-format 3 · SellScreen +2) · e2e 9 เดิมผ่านทั้งหมด (ปุ่ม `nav-stock` ใหม่อยู่ก่อน `nav-shift` ไม่แทนที่ปุ่มเดิม)
...
git commit -m "feat(pos): stock page, stock alert badge and expired-base banner"
```
...
**Files:**
- Create: `apps/pos/src/screens/ReceiveScreen.tsx`, `apps/pos/e2e/stock-receive.spec.ts`
...
- Produces: route `/stock/receive` · testid `receive-supplier`, `receive-item` (value = รหัสสินค้า), `receive-unit`, `receive-qty`, `receive-total`, `receive-add`, `receive-line-<i>`, `receive-jump-<i>`, `receive-remove-<i>`, `receive-sum`, `receive-note`, `receive-paid-drawer` (Q4-6 · ปิดเมื่อไม่มีกะ), `receive-save`, `receive-price-jump`, `receive-confirm-price` (D47-3), `receive-done` · ปุ่ม `nav-receive` บนหน้าสต็อก · `nav-stock` (กลับ)
...
test('receive goods paid from the drawer → stock and X report; a price more than 10% off needs a confirm (spec §5 · D19 · D47-3 · Q4-6)', async ({ page }) => {
  ...
  await expect(page.getByTestId('x-paid-out')).toHaveText('−฿184') // only the first receipt was paid from the drawer
})
```
...
/** "+10.4%" — the deviation of this line's unit cost from the item's current cost, when it is a jump (D47 item 3). */
function jumpText(l: Line): string | null {
  ...
  try {
    ...
    if (!isPriceJump(cost, l.item.avgCostUsat)) return null
    const bp = priceDeviationBp(cost, l.item.avgCostUsat)
    const pct = bp === Number.MAX_SAFE_INTEGER ? '∞' : `${(bp / 100).toFixed(1)}%`
    return `${cost > l.item.avgCostUsat ? '+' : '−'}${pct}`
  } catch {
 ...
 * "paid from the drawer" (Q4-6). Tracked raw items only (D29). A price more than 10% off is flagged on the line and,
 * if saved anyway, refused once with PRICE_JUMP until "ยืนยันราคานี้" (D47 item 3). Receipt photo: plan 5 (Q4-5).
 */
export function ReceiveScreen(): JSX.Element {
  ...
  return (
    <main className="page">
      ...
      <p>
        {TH.receiveSum} <strong data-testid="receive-sum">{formatBaht(lines.reduce((a, l) => a + l.lineTotalSatang, 0))}</strong>
      </p>
      <label>
        {TH.receiveNote}
      ...
      <label className="check">
        <input type="checkbox" data-testid="receive-paid-drawer" checked={paidFromDrawer} disabled={!hasShift} onChange={(e) => setPaidFromDrawer(e.target.checked)} />
      ...
      <div className="actions">
        {priceJump && (
          <button type="button" data-testid="receive-confirm-price" disabled={save.isPending} onClick={() => save.mutate(true)}>
            {TH.receiveConfirmPrice}
          </button>
        )}
        <button type="button" className="primary" data-testid="receive-save" disabled={save.isPending || lines.length === 0} onClick={() => save.mutate(false)}>
          {TH.receiveSave}
        </button>
      </div>
    </main>
...
pnpm --filter @dayo/pos e2e
```
Expected: 11/11 ผ่าน (pos 199) · e2e **10** ผ่าน (9 เดิม + `stock-receive`)

- [ ] **Step 5: Commit**

ใช้ skill `committing-code` แล้ว:
```bash
git add apps/pos/src/screens/ReceiveScreen.tsx apps/pos/e2e/stock-receive.spec.ts apps/pos/src/router.tsx apps/pos/src/screens/StockScreen.tsx
git commit -m "feat(pos): receive goods screen with price-jump confirm and drawer payment"
```
...
- Produces: route `/stock/produce` · testid `produce-base-<code>`, `produce-leftover`, `produce-discard-first` (Q4-9), `produce-scale-5000|10000|15000|20000`, `produce-scale` (พิมพ์เอง เช่น 0.75), `produce-component-<code>`, `produce-yield` (ค่าเริ่มต้น = มาตรฐาน × batch), `produce-save`, `produce-done` · ปุ่ม `nav-produce` บนหน้าสต็อก
    ...
    <main className="page">
      ...
      {base !== null && (
        <>
          ...
          <div className="choices">
            {SCALES_BP.map((bp) => (
              <button key={bp} type="button" data-testid={`produce-scale-${bp}`} aria-pressed={scaleBp === bp} onClick={() => setScale(bp)}>
                × {qtyText(bp / 10)}
              </button>
            ))}
            <label>
...
pnpm --filter @dayo/pos e2e
```
Expected: 11/11 ผ่าน (pos 199) · e2e **11** ผ่าน · ถ้า `stock-row-PB-TEA-THAI` ยังเป็น `out` หลังขาย แปลว่า Task 8 ไม่ได้ invalidate `stockKey` ใน `useCommitSale`
...
git commit -m "feat(pos): produce base screen with batch size, yield and leftover discard"
```
...
pnpm --filter @dayo/pos e2e
```
Expected: 11/11 ผ่าน (pos 199) · e2e **12** ผ่าน
...
git commit -m "feat(pos): stock-out screen by item or by drink with a reason"
```

---

### Task 12: หน้านับสต็อก (`/stock/count`) นับแบบไม่เห็นยอดบัญชี · ชุดหลัก/ทั้งหมด · นับต่อหลังรีโหลด + e2e 💰 review: opus
...
**Interfaces:**
- Consumes: `startStockCount`, `getOpenStockCount`, `saveCountLine`, `removeCountLine`, `closeStockCount` (Task 7) · `stockOverview` (`isKeyCount` — Q4-2) · `formatQty`, `parseQtyInput`, `formatBaht` · `stockKey`, `stockCountKey`, `bootstrapKey` · e2e `firstRun`, `login`
- Produces: route `/stock/count` · testid `count-start`, `count-scope-key|all`, `count-row-<code>`, `count-unit-<code>`, `count-input-<code>`, `count-save-<code>`, `count-counted-<code>`, `count-variance-<code>`, `count-opening-<code>`, `count-recount-<code>`, `count-remove-<code>`, `count-total-variance`, `count-close`, `count-close-confirm`, `count-closed` · ปุ่ม `nav-count` บนหน้าสต็อก (ข้อความ "นับสต็อกต่อ" เมื่อมีใบนับเปิด) · **ไม่มีตัวเลขยอดบัญชีบนจอก่อนบันทึกรายการนั้น (Q4-3)**
...
const item = (code: string, isKeyCount: boolean): StockItemDto => ({
  itemId: `i-${code}`, code, name: code, kind: 'raw', category: 'x', useUnit: 'g', onHandMilli: 800_000, avgCostUsat: 19_250_000, valueSatang: 15_400,
...
const OVERVIEW: StockOverviewDto = {
  ...
  alertCount: 0, expiredBaseCodes: [], lastCountAt: null, countDue: true, openCountId: 'c1',
}
...
describe('CountScreen (spec §4.5 · Q4-3 blind count)', () => {
  it('shows the key items only, with no book figure before a line is saved; the variance appears after saving', async () => {
    ...
    fireEvent.change(screen.getByTestId('count-input-RM-TEA-01'), { target: { value: '1.5' } })
    ...
    expect(api.saveCountLine).toHaveBeenCalledWith({ actorUserId: 'u1', countId: 'c1', itemId: 'i-RM-TEA-01', purchaseUnitId: 'u-RM-TEA-01', countedUnitsMilli: 1_500 })
  ...
  it('refuses a bad number on screen without calling the API', async () => {
    const api = mount()
    await waitFor(() => expect(screen.getByTestId('count-input-RM-TEA-01')).toBeTruthy())
    fireEvent.change(screen.getByTestId('count-input-RM-TEA-01'), { target: { value: '1.2345' } })
    act(() => screen.getByTestId('count-save-RM-TEA-01').click())
...
test('first count = opening balance, blind until saved, resumed after a reload, closed → stock (spec §4.5 · D30 · Q4-2/3/12)', async ({ page }) => {
  await firstRun(page)
  await page.getByTestId('nav-stock').click()
...
function CountRow({ item, line, busy, onSave, onRemove }: { item: StockItemDto; line: StockCountLineDto | undefined; busy: boolean; onSave: (purchaseUnitId: string | null, unitsMilli: number) => void; onRemove: () => void }): JSX.Element {
  const [unitId, setUnitId] = useState<string>(line?.purchaseUnitId ?? item.units.find((u) => u.isDefault)?.id ?? '')
  const [text, setText] = useState('')
  const [editing, setEditing] = useState(line === undefined)
  const [error, setError] = useState<string | null>(null)
  const unitName = item.units.find((u) => u.id === unitId)?.name ?? item.useUnit
  ...
  const save = (): void => {
    ...
    setError(null)
    setEditing(false)
    onSave(unitId === '' ? null : unitId, milli)
  }
  ...
  return (
    <li className="list" data-testid={`count-row-${item.code}`}>
      ...
      {line !== undefined && !editing ? (
        <span>
          <span data-testid={`count-counted-${item.code}`}>{TH.countCounted(formatQty(line.countedUnitsMilli, line.unitName))}</span> ·{' '}
        ...
        <span className="choices">
          <select data-testid={`count-unit-${item.code}`} value={unitId} onChange={(e) => setUnitId(e.target.value)}>
            {item.units.map((u) => (
              <option key={u.id} value={u.id}>
                {u.name} ({formatQty(u.qtyPerUnitMilli, item.useUnit)})
              </option>
          ...
          <input data-testid={`count-input-${item.code}`} inputMode="decimal" aria-label={`${item.name} (${unitName})`} value={text} onChange={(e) => setText(e.target.value)} />
...
/**
 * นับสต็อก (spec §4.5 · §5): one open count per device, resumed after a lock or reload. Count what is on the shelf,
 * in purchase units (3.5 ถุง) or the use unit, item by item — the book figure and the variance show only after a line
 * is saved (Q4-3). "ปิดใบนับ" writes COUNT_ADJ / OPENING (D30) for every counted line. Everyone signed in.
 */
export function CountScreen(): JSX.Element {
  ...
  const saveLine = useMutation({
    mutationFn: (v: { countId: string; itemId: string; purchaseUnitId: string | null; countedUnitsMilli: number }) => api.saveCountLine({ actorUserId, ...v }),
    onSuccess: setCount,
    onError,
  })
  ...
  const items = stock.data.items.filter((i) => scope === 'all' || i.isKeyCount)
          ...
          <ul className="list">
            {items.map((i) => (
              <CountRow
                ...
                busy={busy}
                onSave={(purchaseUnitId, countedUnitsMilli) => saveLine.mutate({ countId: open.id, itemId: i.itemId, purchaseUnitId, countedUnitsMilli })}
...
pnpm --filter @dayo/pos e2e
```
Expected: 11/11 ผ่าน · pos 199 → **201** (CountScreen 2) · e2e **13** ผ่าน
...
git commit -m "feat(pos): blind stock count screen with resume and close"
```
...
```
Expected: 11/11 tasks ผ่าน — contracts 5 · domain 172 · excel-import 33 · db-schema 121 · pos 201 (**532 unit**) · e2e **13** ผ่าน · `git status` ไม่มี `test-results/`, `playwright-report/`, `dist/`, `.superpowers/` ค้างใน index · คำสั่ง grep พิมพ์ `0` · diff ของ `drizzle/` มีแค่ไฟล์ของ `stock_adjustment` (migration เดิมไม่ถูกแก้)
...
1. **ยอดยกมา (D30):** นับสต็อก "ทั้งหมด" ครั้งแรกด้วยของจริงบนชั้น 35 รายการ → ปิดใบนับ → หน้าสต็อกตรงกับของจริง · จดเวลาที่ใช้
...
|---|---|---|---|---|
| 1 | นับยอดยกมาครั้งแรก 35 รายการ | | เวลาที่ใช้ | |
...
## 2. คำตอบของเจ้าของที่ใช้ (Q4-1 … Q4-12 → D56) และคำถามใหม่ระหว่างทำ
...
- แผน 7: รายงานส่วนต่างสต็อก (§4.5) อ่าน `stock_count_line.variance_satang` ของใบนับที่ปิดแล้ว · รายงานของเสียแยกตาม `stock_adjustment.reason_code` (ของเสีย/หมดอายุ/ทดลอง/แจก/อื่นๆ — movement ของแจกและอื่นๆ เป็น `WASTE`) · มูลค่าสต็อกใช้ `stockValueSatang` เดียวกัน
...
git commit -m "docs(plan-4): execution notes and tablet checklist deferred to the final round (d51)"
```
...
| Q4-3 นับแบบไม่เห็นยอด | ไม่เห็นยอดบัญชีจนบันทึกรายการนั้น แล้วเห็นส่วนต่างทันที นับใหม่ได้ | Task 12 `CountRow` · `CountScreen.test.tsx` · e2e `count-variance` ไม่มีก่อนบันทึก |
...
| Q4-6 จ่ายค่าของจากลิ้นชัก | ช่องติ๊ก (ไม่ติ๊กเป็นค่าเริ่มต้น) → `PAID_OUT` ธุรกรรมเดียวกัน ต้องมีกะ | Task 4 `paidFromDrawer` + เทสต์ · Task 9 `receive-paid-drawer` + e2e `x-paid-out` |
...
| Q4-10 ที่แสดงคำเตือน | ปุ่ม "สต็อก ⚠ N" + แถบแดงเบสหมดอายุบนหน้าขาย · "ใกล้หมดอายุ" (≤ 60 นาที) เฉพาะหน้าสต็อก · ไม่มี popup ไม่บล็อก | Task 1 `EXPIRY_SOON_MINUTES` · Task 3 `alert` · Task 8 `nav-stock`, `base-expired` + เทสต์ |
| Q4-11 เบสใกล้หมด | ไม่มีสถานะใกล้หมดสำหรับเบส · เตือนเฉพาะติดลบ/หมดอายุ (เบส = 0 ไม่นับเป็นคำเตือน) | Task 3 `alert` ของ prepared + เทสต์ `alertCount: 31` |
...
- เทสต์ตรงกับโค้ดของ task เดียวกันทุกข้อ: Task 3 เทสต์ใช้แค่ `stockOverview` · Task 4 ใช้ `stockOverview` ไม่ได้ใช้ · Task 5 เทสต์ใช้ `stockOverview` (Task 3) · Task 6 เทสต์ใช้ `produceBatch` (Task 5) · Task 7 เทสต์ใช้ `sellSku`/`stockOverview` · Task 8 unit test ใช้ mock · e2e ของ Task 9–12 ใช้เฉพาะหน้าจอที่มีแล้ว (Task 10 ใช้ปุ่มทิ้งของ Task 8 · Task 9 ใช้ `nav-shift`/`x-paid-out` ของแผน 3b)
